...
import { useSyncProgress, type SyncProgressState } from '@/hooks/use-sync-progress'
...
export function SyncProvider({ children }: { children: ReactNode }) {
  ...
  const { activeAccounts } = useGMBConnection()

  // Get sync queue status
  const { isSyncing: isQueueSyncing, lastJob } = useSyncStatus(userId || undefined)

  // Get real-time sync progress
  const { progress: syncProgress, startTracking, stopTracking, etaMs } = useSyncProgress()
  ...
  }, [isQueueSyncing, activeAccountId, syncProgress, startTracking, stopTracking, lastJob])
  ...
  const mapProgressToStatus = useCallback(
    (progress: SyncProgressState | null): GlobalSyncStatus => {
      const defaultStatus: GlobalSyncStatus = {
        ...
        error: progress?.error || lastJob?.status === 'failed' ? 'Sync failed' : null,
        progress: {
          locations: 'pending',
          reviews: 'pending',
          questions: 'pending',
      ...
      if (stage === 'init') {
        ...
        progressStates.locations =
          status === 'running' ? 'syncing' : status === 'completed' ? 'done' : 'error'
