...
type ElementEventHandler<K extends keyof HTMLElementEventMap> = (event: HTMLElementEventMap[K]) => void;
type DocumentEventHandler<K extends keyof DocumentEventMap> = (event: DocumentEventMap[K]) => void;
...
export function useWindowEventListener<K extends keyof WindowEventMap>(
  ...
  useEffect(() => {
    ...
    // Add event listener
    window.addEventListener(eventType, eventListener as any, options);

    // Remove event listener on cleanup
    return () => {
      window.removeEventListener(eventType, eventListener as any, options);
    };
  }, [eventType, options]);
}
...
export function useDocumentEventListener<K extends keyof DocumentEventMap>(
  ...
  useEffect(() => {
    ...
    document.addEventListener(eventType, eventListener as any, options);

    return () => {
      document.removeEventListener(eventType, eventListener as any, options);
    };
  }, [eventType, options]);
}
...
>(
  ...
  useEffect(() => {
    ...
    element.addEventListener(eventType, eventListener as any, options);

    return () => {
      element.removeEventListener(eventType, eventListener as any, options);
    };
...
export function useEventListeners<T extends HTMLElement = HTMLElement>(
  ...
  useEffect(() => {
    ...
    savedListeners.current.forEach(({ type, handler, options }) => {
      const listener = (event: any) => handler(event);
