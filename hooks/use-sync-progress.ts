...
export type SyncProgressStage =
  ...
  | "queued"
  | "locations"
  | "locations_fetch" // Legacy alias
  | "reviews"
  | "reviews_fetch" // Legacy alias
  | "questions"
  | "questions_fetch" // Legacy alias
  | "posts"
  | "posts_fetch" // Legacy alias
  | "media"
  | "media_fetch" // Legacy alias
  | "performance"
  | "transaction"
  | "completing"
  | "cache_refresh"
  | "complete"
  | "completed" // Alias for complete
  | "error";
  ...
  const etaMs =
    ...
    progress.percentage < 100
      ? Math.max(
          0,
          Math.round(
            (elapsedMs / progress.percentage) * (100 - progress.percentage),
          ),
        )
      : null;
