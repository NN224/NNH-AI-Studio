...
/**
 * Hook for fetching and managing notifications with Realtime support
 */
export function useNotifications(userId?: string) {
  ...
  const { data, isLoading, error, refetch } = useQuery<NotificationsResponse>({
    ...
    refetchInterval: false, // Disable polling - using Realtime instead
  ...
  useEffect(() => {
    ...
    channelRef.current = supabase
      ...
      .on(
        ...
        () => {
          // Invalidate query on updates (e.g., mark as read)
          queryClient.invalidateQueries({ queryKey: ['notifications'] })
        },
      )
      .on(
 ...
 */
function inferPriority(type: string, metadata: Record<string, unknown>): Notification['priority'] {
  // High priority types
