// @ts-expect-error - Deno runtime types are available in Supabase Edge Functions
...
const corsHeaders = {
  ...
  'Access-Control-Allow-Origin': 'https://nnh.ae',
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error(`[GMB Sync] Starting sync for account ${accountId} (${syncType})`)

    // Get Next.js app URL
    const APP_URL =
      Deno.env.get('NEXT_PUBLIC_APP_URL') || Deno.env.get('APP_URL') || 'https://nnh.ae'

    // Call Next.js API endpoint (which has token refresh logic)
    const apiUrl = `${APP_URL}/api/gmb/sync-v2`

    console.error(`[GMB Sync] Calling ${apiUrl}`)
    ...
    if (!response.ok) {
      ...
      let errorJson: { error?: string; message?: string } | null = null

      try {
        errorJson = await response.json()
        errorText = (errorJson?.error || errorJson?.message || errorText) as string
      } catch {
        try {
      ...
      const errorTypeStr = String(errorType)

      if (
        response.status === 401 ||
        errorTypeStr.includes('token') ||
        errorTypeStr.includes('auth')
      ) {
        return new Response(
          JSON.stringify({
            ok: false,
    ...
    console.error(`[GMB Sync] Success for account ${accountId} in ${tookMs}ms`)
