...
export async function fetchLocationsDataForSync(
  ...
  do {
    ...
    for (const rawLocation of googleLocations) {
      ...
      const rating =
        rawLocation.metadata?.starRating ?? rawLocation.profile?.overallStarRating ?? null
      const reviewCount =
        rawLocation.metadata?.totalReviewCount ?? rawLocation.profile?.reviewCount ?? null
  ...
  console.warn('[GMB Sync v2] fetchLocations completed in', Date.now() - locationsTimerStart, 'ms')
  return locations
}
...
export async function fetchReviewsDataForSync(
  ...
  console.warn('[GMB Sync v2] fetchReviews completed in', Date.now() - reviewsTimerStart, 'ms')
  return reviews
}
...
export async function fetchQuestionsDataForSync(
  ...
  console.warn('[GMB Sync v2] fetchQuestions completed in', Date.now() - questionsTimerStart, 'ms')
  return questions
}
...
export async function fetchPostsDataForSync(
    ...
    try {
      ...
      for (const post of googlePosts) {
        ...
        posts.push({
          ...
          media_urls: post.media?.map((m: any) => m.googleUrl || m.sourceUrl).filter(Boolean) || [],
  ...
  console.warn('[GMB Sync v2] fetchPosts completed in', Date.now() - postsTimerStart, 'ms')
  return posts
}
...
export async function fetchMediaDataForSync(
  ...
  console.warn('[GMB Sync v2] fetchMedia completed in', Date.now() - mediaTimerStart, 'ms')
  return media
}
