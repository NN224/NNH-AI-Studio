    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Update multiple business attributes across selected locations at once
          </DialogDescription>
        </DialogHeader>
                ...
                <div key={field.key} className="space-y-2">
                  ...
                  {field.enabled && (
                    <div className="ml-8">
                      ...
                      {field.type === 'select' && field.key === 'priceRange' && (
                        <select
                      ...
                      {field.type === 'features' && (
                        <div className="space-y-2">
                          <p className="text-sm text-muted-foreground">
                            Select features to add (existing features won't be removed)
                          </p>
                          {Object.entries(FEATURE_CATALOG).map(([category, features]) => (
                              ...
                              <div className="flex flex-wrap gap-2">
                                {features.slice(0, 5).map(feature => (
                                  <Badge
                                    ...
                                    onClick={() => {
                                      const current = field.value[category] || [];
                                      const updated = current.includes(feature.key)
                                        ? current.filter((k: string) => k !== feature.key)
                                        : [...current, feature.key];
              ...
              <div className="space-y-4 p-4 bg-secondary/20 rounded-lg">
                <h4 className="font-medium flex items-center gap-2">
                  ...
                  {results.summary.successful > 0 && (
                    <Badge variant="default">{results.summary.successful} Success</Badge>
                  )}
                ...
                {results.results.failed.length > 0 && (
                  <div>
                    <Label className="text-sm text-destructive">Failed updates:</Label>
                    <ul className="mt-1 space-y-1">
