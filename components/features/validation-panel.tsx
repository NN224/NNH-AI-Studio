...
import { BusinessAttributesValidator, type ValidationResult, type BusinessAttributes } from '@/lib/services/business-attributes-validation';
...
export function ValidationPanel({ 
  ...
  const applyAutoFix = () => {
    ...
    try {
      const fixed = BusinessAttributesValidator.autoFix(businessAttributes, validationResult);
  ...
  const autoFixableCount = validationResult.errors.filter(e => e.autoFixable).length;
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center p-3 bg-red-500/10 rounded-lg">
              ...
              <div className="text-lg font-semibold">{validationResult.errors.length}</div>
              <div className="text-xs text-muted-foreground">Errors</div>
            </div>
            <div className="text-center p-3 bg-yellow-500/10 rounded-lg">
              <AlertTriangle className="h-5 w-5 text-yellow-500 mx-auto mb-1" />
              <div className="text-lg font-semibold">{validationResult.warnings.length}</div>
              <div className="text-xs text-muted-foreground">Warnings</div>
            </div>
            <div className="text-center p-3 bg-blue-500/10 rounded-lg">
              <Info className="h-5 w-5 text-blue-500 mx-auto mb-1" />
              <div className="text-lg font-semibold">{validationResult.suggestions.length}</div>
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <Badge 
                          ...
                          className={cn(
                            "text-xs",
                            warning.impact === 'high' && "bg-orange-500/20 text-orange-500",
                            warning.impact === 'medium' && "bg-yellow-500/20 text-yellow-500",
                            warning.impact === 'low' && "bg-blue-500/20 text-blue-500"
                          )}
                        >
                          {warning.impact}
                        </Badge>
              ...
              {validationResult.isValid && 
               ...
               validationResult.suggestions.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-lg font-medium">Profile is perfectly optimized!</p>
