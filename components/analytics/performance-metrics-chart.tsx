  ...
  useEffect(() => {
    async function fetchPerformanceData() {
      try {
        ...
        const clickTypes = ["WEBSITE_CLICKS", "CALL_CLICKS"];

        const conversationTypes = ["BUSINESS_CONVERSATIONS"];

        const bookingTypes = ["BUSINESS_BOOKINGS"];

        const foodTypes = ["BUSINESS_FOOD_ORDERS", "BUSINESS_FOOD_MENU_CLICKS"];

        // Aggregate by date
        const aggregatedByDate: Record<
          string,
          {
            impressions: number;
            clicks: number;
          ...
          <div className="h-[300px] flex flex-col items-center justify-center text-center p-6">
            <div className="text-muted-foreground mb-2">
              ...
              <p className="text-sm mt-2 max-w-md">
                Sync your GMB account to see performance trends based on Google
