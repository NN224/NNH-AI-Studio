...
export function MetricsOverview({
  ...
  useEffect(() => {
    async function fetchMetrics() {
      try {
        ...
        const { data: previousMetrics, error: previousMetricsError } =
          locationIds.length > 0
            ? await supabase!
                ...
                .gte(
                  "metric_date",
                  previousRange.start.toISOString().split("T")[0],
                )
                .lte(
                  "metric_date",
                  previousRange.end.toISOString().split("T")[0],
                )
            : { data: [], error: null };
        ...
        const currentAggregated = aggregateMetricsByType(
          currentMetrics || [],
          currentRange,
        );
        const previousAggregated = aggregateMetricsByType(
          previousMetrics || [],
          previousRange,
        );
        ...
        const totalConversations =
          currentAggregated["BUSINESS_CONVERSATIONS"] || 0;
        const totalClicks =
          (currentAggregated["WEBSITE_CLICKS"] || 0) +
          (currentAggregated["CALL_CLICKS"] || 0);
        const totalDirectionRequests =
          currentAggregated["BUSINESS_DIRECTION_REQUESTS"] || 0;
        ...
        const prevConversations =
          previousAggregated["BUSINESS_CONVERSATIONS"] || 0;
        const prevClicks =
          (previousAggregated["WEBSITE_CLICKS"] || 0) +
          (previousAggregated["CALL_CLICKS"] || 0);
        const prevDirections =
          previousAggregated["BUSINESS_DIRECTION_REQUESTS"] || 0;
    ...
    const channel = supabase!
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "gmb_performance_metrics" },
        fetchMetrics,
      )
      .subscribe();
  ...
  return (
    <div
      className={`grid gap-4 ${totalCards === 4 ? "md:grid-cols-2 lg:grid-cols-4" : totalCards === 5 ? "md:grid-cols-2 lg:grid-cols-5" : "md:grid-cols-2 lg:grid-cols-6"}`}
    >
      <MetricCard
