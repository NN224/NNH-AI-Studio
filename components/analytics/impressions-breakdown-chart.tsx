...
export function ImpressionsBreakdownChart({
  dateRange = "30",
  locationIds,
}: ImpressionsBreakdownChartProps) {
  ...
  if (
    !breakdownData ||
    breakdownData.mapsTotal + breakdownData.searchTotal === 0
  ) {
    return (
      <Card className="bg-card border-primary/30">
        <CardHeader>
          ...
          <div className="h-[300px] flex flex-col items-center justify-center text-center p-6">
            <div className="text-muted-foreground mb-2">
              ...
              <p className="text-sm mt-2 max-w-md">
                Sync your GMB account to see impressions breakdown by device and
                source.
              </p>
            </div>
          </div>
  ...
  const summaryData = [
    {
      ...
      percent:
        breakdownData.mapsTotal + breakdownData.searchTotal > 0
          ? (
              (breakdownData.mapsTotal /
                (breakdownData.mapsTotal + breakdownData.searchTotal)) *
              100
            ).toFixed(1)
          : "0",
    },
    ...
    },
    {
      ...
      percent:
        breakdownData.desktopTotal + breakdownData.mobileTotal > 0
          ? (
              (breakdownData.desktopTotal /
                (breakdownData.desktopTotal + breakdownData.mobileTotal)) *
              100
            ).toFixed(1)
          : "0",
    },
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {summaryData.map((item) => (
            <div
              key={item.label}
              className="text-center p-3 rounded-lg bg-secondary border border-primary/20"
            >
