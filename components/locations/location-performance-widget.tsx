...
export function LocationPerformanceWidget({
  locationId,
  compact = false,
}: LocationPerformanceWidgetProps) {
    ...
    async function fetchMetrics() {
      try {
        ...
        const clicksChange =
          previousClicks > 0
            ? Math.round(
                ((currentClicks - previousClicks) / previousClicks) * 100 * 10,
              ) / 10
    ...
    return (
      <div className="flex items-center gap-4 text-xs">
        ...
        {metrics.impressionsChange !== 0 && (
          <div
            className={`flex items-center gap-1 ${metrics.impressionsChange > 0 ? "text-success" : "text-destructive"}`}
          >
            <TrendingUp
              className={`h-3 w-3 ${metrics.impressionsChange < 0 ? "rotate-180" : ""}`}
            />
            <span>
              {metrics.impressionsChange > 0 ? "+" : ""}
              {metrics.impressionsChange.toFixed(0)}%
            </span>
          </div>
        )}
      </div>
      ...
      <CardContent className="p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            ...
            {metrics.impressionsChange !== 0 && (
              <p
                className={`text-xs ${metrics.impressionsChange > 0 ? "text-success" : "text-destructive"}`}
              >
                {metrics.impressionsChange > 0 ? "+" : ""}
          ...
          <div>
            ...
            {metrics.clicksChange !== 0 && (
              <p
                className={`text-xs ${metrics.clicksChange > 0 ? "text-success" : "text-destructive"}`}
              >
                {metrics.clicksChange > 0 ? "+" : ""}
          ...
          {metrics.bookings > 0 && (
            <div>
              ...
              <p className="text-lg font-semibold text-foreground">
                {metrics.bookings.toLocaleString()}
              </p>
            </div>
          )}
          ...
          {metrics.foodOrders > 0 && (
            <div>
              ...
              <p className="text-lg font-semibold text-foreground">
                {metrics.foodOrders.toLocaleString()}
              </p>
            </div>
          )}
        </div>
