...
export function LocationsMapTab() {
  ...
  const [aiContextLocation, setAiContextLocation] = useState<Location | null>(
    null,
  );
  ...
  const [pendingVariant, setPendingVariant] = useState<BrandingVariant | null>(
    null,
  );
  const [uploadingVariant, setBrandingUploadVariant] =
    useState<BrandingVariant | null>(null);
  ...
  const allLocationIds = useMemo(
    () => locationsWithGeo.map((loc) => loc.id),
    [locationsWithGeo],
  );
  const {
    stats: _stats,
    ...
    error: statsError,
  } = useLocationMapData(selectedLocationId);
  ...
  const attentionLocations = useMemo(() => {
    return locationsWithGeo
      ...
      .sort(
        (a, b) =>
          (b.pendingReviews ?? 0) +
          (b.pendingQuestions ?? 0) -
          ((a.pendingReviews ?? 0) + (a.pendingQuestions ?? 0)),
      )
      .slice(0, 3);
  }, [locationsWithGeo]);
  ...
  const aiStatusDisplay = useMemo(
    () => deriveDisplayStatus(aiLocation),
    [aiLocation],
  );
  const aiRecommendations = useMemo(() => {
    ...
    if (aiLocation) {
      if ((aiLocation.pendingReviews ?? 0) > 0) {
        suggestions.push(
          `Reply to ${aiLocation.pendingReviews} pending reviews for ${aiLocation.name}.`,
        );
      }

      if ((aiLocation.pendingQuestions ?? 0) > 0) {
        suggestions.push(
          `Answer ${aiLocation.pendingQuestions} open questions to increase trust.`,
        );
      }

      if ((aiLocation.responseRate ?? 0) < 70) {
        suggestions.push(
          "Aim for a response rate above 70% to boost local ranking.",
        );
      }
    }
    ...
    if (attentionLocations.length > 0) {
      ...
      suggestions.push(
        `Prioritize ${attention.name} — it has outstanding actions that need attention.`,
      );
    }
    ...
    if (topPerformers.length > 0) {
      ...
      suggestions.push(
        `Use ${top.name} as a playbook — replicate its content cadence and review replies.`,
      );
    }
  ...
  const businessStatus = useMemo(
    () => deriveBusinessStatus(selectedLocation ?? null),
    [selectedLocation],
  );
  ...
  const buildCompetitorSearchParams = useCallback((context: Location) => {
    ...
    const additionalCategories: Array<{ displayName?: string | null }> =
      Array.isArray(categories?.additionalCategories)
    ...
    const categoryLabel =
      (typeof primaryCategory?.displayName === "string" &&
        primaryCategory.displayName) ??
      (typeof context.category === "string" && context.category.length > 0
        ? context.category
        : null);
    if (categoryLabel) {
    ...
    const metadataKeywords: unknown =
      metadata?.keywords ?? metadata?.topKeywords ?? metadata?.tags;
  ...
  const fetchCompetitorsForLocation = useCallback(
    async (context: Location | null | undefined) => {
      ...
      const response = await fetch(
        `/api/locations/competitors?${params.toString()}`,
        {
          method: "GET",
          credentials: "include",
        },
  ...
  const {
    ...
    enabled:
      aiPanelOpen &&
      aiView === "competitors" &&
      Boolean(aiContextLocation?.coordinates),
    staleTime: 1000 * 60 * 5,
  });
  ...
  const competitors = useMemo(() => {
    ...
    return (
      (competitorsResponse.competitors as Array<Record<string, any>>) ?? []
    );
  }, [competitorsResponse]);
  ...
  const previewCompetitors = useMemo(() => {
    if (
      ...
      Array.isArray(previewCompetitorsResponse)
    ) {
      return previewCompetitorsResponse ?? [];
    }
    return (
      (previewCompetitorsResponse.competitors as Array<Record<string, any>>) ??
      []
    );
  }, [previewCompetitorsResponse]);
  ...
  const locationsAggregateOverview = useMemo(() => {
    ...
    for (const loc of locations) {
      const rating =
        typeof loc.rating === "number" && Number.isFinite(loc.rating)
          ? loc.rating
          : null;
      if (rating !== null) {
        ratingSum += rating;
      ...
      const healthScore =
        typeof loc.healthScore === "number" && Number.isFinite(loc.healthScore)
          ? loc.healthScore
          : null;
      if (healthScore !== null) {
      ...
      const reviews =
        typeof loc.reviewCount === "number" && Number.isFinite(loc.reviewCount)
          ? loc.reviewCount
          : 0;
      reviewTotal += reviews;
    }
    ...
    return {
      ...
      avgHealthScore:
        healthScoreCount > 0 ? healthScoreSum / healthScoreCount : null,
    };
  }, [locations]);
  ...
  const statsOverviewDisplays = useMemo(() => {
    const avgRating =
      typeof statsOverview.avgRating === "number" &&
      Number.isFinite(statsOverview.avgRating)
    ...
    const avgHealthScore =
      typeof statsOverview.avgHealthScore === "number" &&
      Number.isFinite(statsOverview.avgHealthScore)
  ...
  const handleBrandingFileChange: React.ChangeEventHandler<
    HTMLInputElement
  > = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/locations/${selectedLocation.id}/branding`,
        {
          method: "POST",
          body: formData,
        },
      ...
      toast.success(
        variant === "cover" ? "Cover photo updated" : "Logo updated",
      );
      await refetch();
    } catch (error: unknown) {
      const message =
  ...
  const openAiPanel = useCallback(
    (
      ...
      view: "insights" | "competitors" = "insights",
    ) => {
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Unable to sync locations right now.";
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "location:select",
      handleLocationSelect as EventListener,
    );
    return () => {
  ...
  const { data: locationStatsResponse, isFetching: locationStatsLoading } =
    useQuery({
      queryKey: ["location-stats", selectedLocationId],
      queryFn: async () => {
        if (!selectedLocationId) return null;
        const response = await fetch(
          `/api/locations/${selectedLocationId}/stats`,
          {
            method: "GET",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
  ...
  const locationStats =
    locationStatsResponse?.data ?? locationStatsResponse ?? null;
  const pendingReviews =
    locationStats?.pendingReviews ?? selectedLocation?.pendingReviews ?? 0;
  const pendingQuestions =
    locationStats?.pendingQuestions ?? selectedLocation?.pendingQuestions ?? 0;
  const lastReviewAt =
    locationStats?.lastReviewAt ?? selectedLocation?.lastReviewAt ?? null;
          ...
          <div className="text-center">
            ...
            <div className="text-xs text-muted-foreground bg-muted p-3 rounded-md text-left max-w-md mx-auto">
              ...
              <p className="font-mono text-xs break-all">
                {locationsError.name}: {locationsError.message}
              </p>
            </div>
            <button
              ...
              <div className="pointer-events-none absolute inset-0 flex items-center justify-center bg-black/50">
                <div className="rounded-2xl border border-white/15 bg-black/80 px-6 py-4 text-center">
                  ...
                  <p className="mt-1 text-xs text-white/60">
                    Tap any pin on the map to preview details, metrics, and
                    quick actions.
                  </p>
                </div>
              </div>
            ...
            {selectedLocation ? (
              <div className="space-y-6">
                  ...
                  {resolvedCoverImage ? (
                    ...
                    <div className="flex h-full w-full flex-col items-center justify-center gap-3 bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-900 text-white">
                      ...
                      <p className="text-xs text-white/60">
                        Add a hero image to showcase your location.
                      </p>
                    </div>
                  )}
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          router.push(
                            `/analytics?location=${selectedLocation.id}`,
                          )
                        }
                      >
                ...
                {statsError && (
                  <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-200">
                    Couldn’t refresh live stats. Showing cached dashboard
                    numbers instead.
                  </div>
                )}
              </div>
              ...
              <div className="flex min-h-[200px] flex-col items-center justify-center text-center text-white/60">
                ...
                <p className="mt-2 max-w-sm text-sm">
                  Choose a pin on the map to preview branding and performance
                  details.
                </p>
              </div>
            )}
        ...
        <aside className="flex h-full flex-col rounded-[24px] border border-white/15 bg-black/40 p-5 backdrop-blur">
          ...
          >
            {locations.map((location) => {
              const rating =
                location.rating != null && !Number.isNaN(location.rating)
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <Badge
                        variant="outline"
                        className={cn("text-xs", statusDisplay.badgeClass)}
                      >
          ...
          <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-white">
            <div className="flex items-center justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-white/60">
                  Auto-discovered using the selected location.
                </p>
              </div>
              {selectedLocation ? (
                <div className="flex items-center gap-2">
            ...
            <div className="mt-3 space-y-3 text-xs text-white/70">
              {!selectedLocation && (
                <p className="text-white/60">
                  Select a location to surface nearby competitors automatically.
                </p>
              )}
              ...
              {selectedLocation && !selectedLocation.coordinates && (
                <div className="space-y-2">
                  <p className="text-white/60">
                    Add a precise address or map pin so we can locate
                    competitors around this business.
                  </p>
                  <Button
                    size="sm"
                    variant="outline"
              ...
              {selectedLocation && selectedLocation.coordinates && (
                <>
                            ...
                            >
                              <div className="flex items-start justify-between gap-2">
                                ...
                                <div className="text-right text-xs text-white/70">
                                  ...
                                  <p>
                                    {competitor.userRatingsTotal ?? 0} reviews
                                  </p>
                                </div>
                              </div>
                              <div className="mt-2 flex flex-wrap items-center gap-2 text-white/60">
                                <span>
                                  Status:{" "}
                                  {competitor.businessStatus ?? "Unknown"}
                                </span>
                  ...
                  {!previewCompetitorsError &&
                    ...
                    previewCompetitors.length === 0 && (
                      <p className="text-white/60">
                        No competitors detected within the default search radius
                        yet.
                      </p>
                    )}
                </>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="rounded-[24px] border border-white/15 bg-black/35 p-5 backdrop-blur">
          ...
          <p className="mb-4 text-xs text-white/60">
            Workflows open with the currently selected location.
          </p>
          <div className="space-y-2">
            <Button
              variant="outline"
              ...
              onClick={() =>
                selectedLocation &&
                router.push(`/reviews?location=${selectedLocation.id}`)
              }
            ...
            <Button
              ...
              onClick={() =>
                selectedLocation &&
                router.push(`/posts?location=${selectedLocation.id}`)
              }
              disabled={quickActionDisabled}
            >
              <span className="flex items-center gap-2 text-sm">
            ...
            <Button
              ...
              onClick={() =>
                selectedLocation &&
                router.push(`/analytics?location=${selectedLocation.id}`)
              }
              disabled={quickActionDisabled}
            >
              <span className="flex items-center gap-2 text-sm">
          ...
          {quickActionDisabled && (
            <p className="mt-3 text-xs text-white/45">
              Select a location on the map to enable these actions.
            </p>
          )}
        </section>
        ...
        <section className="rounded-[24px] border border-white/15 bg-black/35 p-5 backdrop-blur">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm text-white">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <p className="mt-2 text-xl font-semibold">
                {statsOverviewDisplays.avgHealthScore}
              </p>
            </div>
          </div>
        </section>
                ...
                <div className="rounded-xl border border-white/10 bg-black/40 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-white/60">
                        {aiLocation.address ?? "No address on file"}
                      </p>
                    </div>
                    <Badge
                      variant="outline"
                      className={cn("text-xs", aiStatusDisplay.badgeClass)}
                    >
                  ...
                  <div>
                    ...
                    <div className="mt-2 space-y-2">
                      {topPerformers.length > 0 ? (
                        topPerformers.map((location) => (
                          ...
                          >
                            ...
                            <div className="mt-1 flex flex-wrap gap-3 text-white/60">
                              ...
                              <span>
                                Response:{" "}
                                {Math.round(location.responseRate ?? 0)}%
                              </span>
                            </div>
                          </div>
                        ))
                        ...
                        <p className="text-xs text-white/60">
                          Add more activity to surface performance leaders.
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="mt-2 space-y-2">
                      {attentionLocations.length > 0 ? (
                          ...
                          >
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <span className="text-amber-200">
                                🟡 {location.pendingReviews ?? 0} reviews ·{" "}
                                {location.pendingQuestions ?? 0} questions
                              </span>
                            </div>
                            <div className="mt-2 flex flex-wrap gap-2">
                              ...
                              <Button
                                ...
                                className="h-7 px-2 text-xs text-purple-300 hover:text-purple-200"
                                onClick={() =>
                                  openAiPanel(location, "competitors")
                                }
                              >
                                Compare competitors
                              </Button>
                        ...
                        <p className="text-xs text-white/60">
                          No locations require urgent attention.
                        </p>
                      )}
                    </div>
                  </div>
                    ...
                    <ul className="mt-2 space-y-2 text-xs text-white/70">
                      {aiRecommendations.length > 0 ? (
                        ...
                        <li className="text-white/60">
                          Keep up the great work — AI will surface new
                          opportunities soon.
                        </li>
                      )}
                    </ul>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {!competitorsLoading && competitors.length === 0 && (
                      <p className="text-xs text-white/60">
                        No competitor data found for this location. Try syncing
                        or adjusting the radius.
                      </p>
                    )}
                    {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                    {competitors.map((competitor: any) => (
                      ...
                      >
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          <span className="text-xs text-white/60">
                            Status: {competitor.businessStatus ?? "Unknown"}
                          </span>
                          {competitor.openNow != null && (
                            <span className="text-xs text-white/60">
...
function deriveBusinessStatus(location: Location | null) {
  ...
  if (
    ...
    const todaysPeriods = regularHours.periods.filter((period: any) => {
      const openDay = (period.openDay ?? period.open_day ?? "")
        .toString()
        .toUpperCase();
...
function formatPeriodTime(
  time: { hours?: number; minutes?: number } | Record<string, any>,
) {
  if (!time) return "--";
