    ...
    async function fetchLocationMedia() {
      try {
        ...
        const response = await fetch(
          `/api/gmb/media?locationId=${location.id}`,
        );
        ...
        if (result.data?.media) {
          ...
          media.forEach((item: any) => {
            const category =
              ...
              item.category;
            const url =
              item.sourceUrl || item.googleUrl || item.url || item.thumbnailUrl;
          ...
          if (!foundCover && media.length > 0) {
            ...
            if (firstPhoto) {
              setCoverPhoto(
                firstPhoto.sourceUrl ||
                  firstPhoto.googleUrl ||
                  firstPhoto.url ||
  ...
  const formatHours = (hours: any) => {
    ...
    return hours.periods
      .map((period: any) => {
        const openDay =
          dayAbbr[
            period.openDay === "SUNDAY"
              ? 0
              : period.openDay === "MONDAY"
                ? 1
                : period.openDay === "TUESDAY"
                  ? 2
                  : period.openDay === "WEDNESDAY"
                    ? 3
                    : period.openDay === "THURSDAY"
                      ? 4
                      : period.openDay === "FRIDAY"
                        ? 5
                        : 6
          ];
        const openTime = period.openTime
          ? `${period.openTime.hours || 0}:${String(period.openTime.minutes || 0).padStart(2, "0")}`
          : "";
        const closeTime = period.closeTime
          ? `${period.closeTime.hours || 0}:${String(period.closeTime.minutes || 0).padStart(2, "0")}`
          : "";
        ...
        <Card className="bg-card border-primary/30 hover:border-primary/50 transition-all duration-300 hover:shadow-lg hover:shadow-primary/20 overflow-hidden">
          ...
          <div className="absolute top-4 right-4 z-10">
            {location.is_syncing ? (
              <motion.div
                ...
                className="w-3 h-3 rounded-full bg-linear-to-r from-primary to-accent"
              />
            ) : (
          ...
          <CardContent className="p-6 relative z-10">
                ...
                <div className="mt-3 pt-3 border-t border-primary/20">
                  ...
                  {childrenLocations.length > 0 && (
                    <div className="text-xs text-muted-foreground">
                      {childrenLocations.length} child location
                      {childrenLocations.length > 1 ? "s" : ""}
                    </div>
                  )}
                </div>
              )}
            ...
            {(mapsUri || newReviewUri || canHaveFoodMenus) && (
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                {canHaveFoodMenus && (
                  <Badge
                    variant="outline"
                    className="text-xs bg-secondary/50 border-primary/30"
                  >
            ...
            {location.ai_insights && (
              <div className="p-3 rounded-lg bg-linear-to-r from-primary/20 to-accent/20 border border-primary/30 mb-4">
