...
export function SmartAISuggestions({
  ...
  useEffect(() => {
    const loadDismissed = async () => {
      try {
        ...
        if (!errorMessage.includes('user_suggestion_actions')) {
          aiLogger.error('Failed to load dismissed suggestions', new Error(errorMessage))
        }
  ...
  const generateSmartSuggestions = (): SmartSuggestion[] => {
    ...
    if (!hasAutoReply && totalReviews > 10) {
      suggestions.push({
        ...
        description: 'Save time with AI-powered automatic responses to common reviews.',
        impact: 'high',
    ...
    if (totalReviews === 0) {
      suggestions.push({
        ...
        description: 'Connect your Google Business Profile to start managing reviews.',
        impact: 'high',
  ...
  const dismissSuggestion = async (id: string) => {
    ...
    try {
      ...
      // Fallback to localStorage
      localStorage.setItem(`dismissed_suggestions_${userId}`, JSON.stringify(newDismissed))
  ...
  const handleAction = async (suggestion: SmartSuggestion) => {
    ...
    try {
      ...
      aiLogger.error('Failed to track click', err instanceof Error ? err : new Error(String(err)))
    }
  ...
  return (
    <Card data-tour="ai-suggestions" className="border-border/50 bg-card/50 overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-border/50">
        ...
        <Button variant="ghost" size="icon" onClick={refreshSuggestions} disabled={refreshing}>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <Badge
                          ...
                          className={cn(
                            ...
                            suggestion.impact === 'low' && 'border-green-500/50 text-green-500',
                          )}
                        >
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        size="sm"
                        className={cn('h-8 text-xs gap-1', colors.bg, colors.text, colors.hover)}
                        variant="ghost"
