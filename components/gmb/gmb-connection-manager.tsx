...
export function GMBConnectionManager({
  ...
  const [disconnectOption, setDisconnectOption] =
    useState<DisconnectOption>("keep");
  ...
  useEffect(() => {
    ...
    const handleConnectionEvent = () => {
      if (isMounted.current) refreshGmbStatus();
    };
  ...
  }, [realtimeProgress, showRealtimeProgress, stopSyncProgress]);
      ...
      toast.error("Connection failed", {
        description:
          error instanceof Error
            ? error.message
            : "Unable to generate the authorization link. Please try again.",
      });
      setConnecting(false);
    }
  };
  ...
  const handleDisconnect = async (e?: React.MouseEvent) => {
    ...
    try {
      const result = await disconnectGMBAccount(
        activeAccount.id,
        disconnectOption,
      );
              ...
              <div className="mt-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {phases.map((p) => (
                    <div
                      key={p.phase}
                      className="rounded-md border border-primary/20 p-2"
                    >
                      <div className="flex items-center justify-between">
  ...
  return (
    <>
      <Card
        className={cn(
          "border-primary/30 bg-linear-to-br from-primary/5 to-accent/5",
          className,
        )}
      >
        ...
        <CardContent>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <AnimatePresence mode="wait">
              {gmbConnected ? (
                ...
                >
                  <Button
                    className="w-full sm:w-auto bg-linear-to-r from-primary to-accent text-white"
                    onClick={handleConnect}
                    disabled={connecting}
                    title="Connect to Google My Business"
                  >
                    {connecting ? (
                      ...
                      <>
                        <Link2 className="h-4 w-4 mr-2" /> Connect Google My
                        Business
                      </>
                    )}
                  </Button>
          ...
          {!useNewSync && progressOpen && gmbConnected && (
            <div className="mt-6 space-y-4">
                ...
                {phases.map((p) => (
                  ...
                  >
                    ...
                    {(() => {
                      if (
                        !p.last_counts ||
                        typeof p.last_counts !== "object" ||
                        p.last_counts === null
                      )
                        return null;
                      const counts = p.last_counts as Record<string, unknown>;
              ...
              {estimateMs > 0 && (
                <p className="text-xs text-muted-foreground">
                  Estimated remaining ~ {Math.round(estimateMs / 1000)}s
                </p>
              )}
              <div className="flex items-center gap-2">
                <Button
        ...
        <div className="py-4">
          <RadioGroup
            ...
            onValueChange={(value: string) =>
              setDisconnectOption(value as DisconnectOption)
            }
          >
            <div className="space-y-3">
              ...
              <div className="flex items-start space-x-3 rounded-lg border border-zinc-700 bg-zinc-800/50 p-4 hover:bg-zinc-800 transition-colors">
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <p className="text-xs text-zinc-500">
                    Anonymize and archive your data for historical analysis.
                    Personal info is removed while stats stay intact.
                  </p>
                </div>
              ...
              <div className="flex items-start space-x-3 rounded-lg border border-zinc-700 bg-zinc-800/50 p-4 hover:bg-zinc-800 transition-colors">
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <p className="text-xs text-zinc-500">
                    Download all data as JSON, then anonymize and archive it for
                    a complete backup.
                  </p>
                </div>
              ...
              <div className="flex items-start space-x-3 rounded-lg border border-red-500/30 bg-red-500/5 p-4 hover:bg-red-500/10 transition-colors">
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <p className="text-xs text-red-300/70">
                    Permanently remove all locations, reviews, questions, and
