...
export default function BusinessHeader({ className }: { className?: string }) {
  ...
  async function fetchFromGMB() {
    if (!loc?.id) return;
    try {
      const res = await fetch(`/api/gmb/media?locationId=${loc.id}`, {
        cache: "no-store",
      });
      ...
      try {
        ...
        // Media update failure should not break the main flow
      }
      ...
      toast({
        title: "Fetch error",
        description: err.message || "Failed to fetch from GMB",
      });
    }
  }
    ...
    (async () => {
      try {
        ...
        const { data: location } = await supabase
          .from("gmb_locations")
          .select(
            "id, location_name, rating, review_count, address, logo_url, cover_photo_url",
          )
          .eq("user_id", user.id)
          .eq("is_active", true)
          .limit(1)
        ...
        if ((!logoUrl || !coverUrl) && loc?.id) {
          try {
            const mediaRes = await fetch(
              `/api/gmb/media?locationId=${loc.id}`,
              {
                cache: "no-store",
              },
            );
            ...
            if (!coverUrl) {
              const cover =
                media.find(isCover) ||
                media.find((m) =>
                  (m?.description || "").toLowerCase().includes("cover"),
                ) ||
            ...
            if (!logoUrl) {
              const logo =
                media.find(isLogo) ||
                media.find((m) =>
                  (m?.description || "").toLowerCase().includes("logo"),
                ) ||
                media.find((m) => (m?.mediaFormat || m?.type) !== "VIDEO"); // first photo fallback
                  ...
                  setLoc((prev) =>
                    prev
                      ? {
                          ...
                          cover_photo_url:
                            uploadOpen === "cover" ? url : prev.cover_photo_url,
                        }
                      : prev,
                  );
                  // Persist to DB
                    ...
                    if (loc?.id) {
                      await fetch("/api/locations/update-media", {
                        ...
                        body: JSON.stringify({
                          ...
                          cover_photo_url:
                            uploadOpen === "cover" ? url : undefined,
                        }),
                      });
                    }
                  } catch {
