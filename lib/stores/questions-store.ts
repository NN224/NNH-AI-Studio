...
interface QuestionsState {
  ...
  setKnowledgeBase: (
    kb: Array<{ id: string; question: string; answer: string }>,
  ) => void;
        ...
        fetchQuestions: async (append = false) => {
          ...
          try {
            ...
            set((state) => {
              ...
              state.hasMore =
                data.hasMore ?? data.questions?.length === state.pageSize;
        ...
        answerQuestion: async (questionId, answer) => {
          try {
            const response = await fetch(
              `/api/gmb/questions/${questionId}/answer`,
              {
                method: "POST",
...
export const selectSelectedQuestion = (state: QuestionsState) =>
