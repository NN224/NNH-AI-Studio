        ...
        fetchReviews: async (append = false) => {
          ...
          try {
            ...
            set((state) => {
              ...
              state.hasMore = data.hasMore ?? (data.reviews?.length === state.pageSize);
              state.reviewsLoading = false;
            });
          } catch (error) {
            set((state) => {
              state.reviewsError = error instanceof Error ? error : new Error('Unknown error');
              state.reviewsLoading = false;
            });
          }
        },
...
export const selectSelectedReview = (state: ReviewsState) => state.selectedReview;
