...
const QuestionAnalysisSchema = z.object({
  category: z.enum(['product', 'service', 'hours', 'location', 'pricing', 'availability', 'general']),
  intent: z.enum(['information', 'complaint', 'comparison', 'recommendation', 'clarification']),
  urgency: z.enum(['low', 'medium', 'high']),
  sentiment: z.enum(['positive', 'neutral', 'negative']),
  topics: z.array(z.string()),
  suggestedAnswer: z.string(),
  confidence: z.number().min(0).max(1),
  relatedFAQs: z.array(z.object({
    question: z.string(),
    answer: z.string(),
    relevance: z.number()
  })).optional()
...
export class MLQuestionsService {
  ...
  private knowledgeBase: Map<string, { question: string; answer: string; category: string }> = new Map();
  ...
  async batchAnalyze(
    ...
    for (let i = 0; i < questions.length; i += batchSize) {
      ...
      const promises = batch.map(q => 
        this.analyzeQuestion(q.text, context, options)
          .then(analysis => ({ id: q.id, analysis }))
      );
   ...
   */
  calculateConfidence(analysis: QuestionAnalysis, context: BusinessContext): number {
...
Response format (JSON):
{
  ...
  private fallbackAnalysis(
    ...
    const suggestedAnswer = this.generateBasicAnswer(question, context, category);
  ...
  private extractTopics(question: string): string[] {
    ...
    const stopWords = new Set(['the', 'is', 'are', 'what', 'when', 'where', 'how', 'why', 'do', 'does', 'can', 'could']);
  ...
  private enhanceWithLocalKnowledge(
    ...
    if (similar.length > 0) {
      ...
      const maxRelevance = Math.max(...analysis.relatedFAQs.map(f => f.relevance));
      if (maxRelevance > 0.8) {
  ...
  private generateKnowledgeKey(question: string): string {
    return question.toLowerCase().replace(/[^\w\s]/g, '').replace(/\s+/g, '_');
  }
  ...
  private findSimilarQuestions(question: string, limit: number = 3): Array<{question: string; answer: string; category: string}> {
    // Simple implementation - in production, use vector similarity
    const results: Array<{question: string; answer: string; category: string; score: number}> = [];
