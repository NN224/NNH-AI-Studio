...
export class MLSentimentService {
  ...
  private constructor() {
    ...
    if (process.env.OPENAI_API_KEY) {
      ...
      aiLogger.warn(
        "No ML API key configured for sentiment analysis - using basic analysis",
      );
      this.apiKey = "";
  ...
  async analyzeSentiment(
    ...
    try {
      ...
      aiLogger.error(
        "ML sentiment analysis failed, falling back to basic analysis",
  ...
  async analyzeBatch(
    ...
    for (let i = 0; i < reviews.length; i += batchSize) {
      ...
      const batchPromises = batch.map((review) =>
        this.analyzeSentiment(review.text, review.rating).then((result) => ({
          id: review.id,
          result,
        })),
      );
      ...
      batchResults.forEach((result, index) => {
        if (result.status === "fulfilled") {
          ...
          results.set(
            review.id,
            this.fallbackAnalysis(review.text, review.rating),
          );
        }
      });
    }
  ...
  private inferAspects(text: string): SentimentAnalysisResult["aspects"] {
    ...
    if (/service|staff|employee|waiter|manager/i.test(text)) {
      aspects.service = this.scoreAspect(
        text,
    ...
    if (/quality|fresh|delicious|tasty/i.test(text)) {
      aspects.quality = this.scoreAspect(
        text,
        ["excellent", "great", "fresh"],
        ["poor", "bad", "stale"],
      );
    }

    if (/price|value|expensive|cheap|worth/i.test(text)) {
      aspects.price = this.scoreAspect(
        text,
        ["reasonable", "fair", "worth"],
        ["expensive", "overpriced", "ripoff"],
      );
    }

    if (/clean|hygiene|dirty|spotless/i.test(text)) {
      aspects.cleanliness = this.scoreAspect(
        text,
    ...
    if (/atmosphere|ambiance|environment|decor/i.test(text)) {
      aspects.atmosphere = this.scoreAspect(
        text,
        ["nice", "cozy", "beautiful"],
        ["loud", "uncomfortable", "cramped"],
      );
    }
  ...
  private extractTopics(text: string): SentimentAnalysisResult["topics"] {
    const topicPatterns = [
      ...
      {
        pattern: /atmosphere|ambiance|decor|environment/i,
        topic: "Atmosphere",
      },
  ...
  static calculateStats(results: SentimentAnalysisResult[]): SentimentStats {
    ...
    const aspectAverages = Object.entries(aspectScores).map(
      ([aspect, scores]) => ({
        aspect,
