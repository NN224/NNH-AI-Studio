...
export class BusinessAttributesValidator {
  ...
  private static validateFormats(
    ...
    if (attributes.phone) {
      const phonePattern =
        PHONE_PATTERNS[country as keyof typeof PHONE_PATTERNS] ||
        PHONE_PATTERNS.DEFAULT;
    ...
    if (
      attributes.email &&
      !z.string().email().safeParse(attributes.email).success
    ) {
      errors.push({
        field: "email",
  ...
  private static validateContent(
    ...
    if (attributes.shortDescription) {
      ...
      if (attributes.shortDescription.length < 50) {
        warnings.push({
          field: "shortDescription",
          message:
            "Short description is too brief. Consider adding more details.",
          impact: "medium",
        });
      }
      ...
      const uniqueWords = new Set(
        attributes.shortDescription.toLowerCase().split(" "),
      ).size;
    ...
    const hasArabicDescription = /[\u0600-\u06FF]/.test(
  ...
  private static calculateScore(
    ...
    warnings.forEach((warning) => {
      score -=
        warning.impact === "high" ? 5 : warning.impact === "medium" ? 3 : 1;
    });

    // Bonus for completeness
    if (attributes.description && attributes.description.length > 100)
      score += 5;
    if (
      attributes.businessHours &&
      Object.keys(attributes.businessHours).length >= 7
    )
      score += 5;
    if (
      attributes.features &&
      Object.values(attributes.features).flat().length > 10
    )
      score += 5;
    if (
      attributes.socialLinks &&
      Object.keys(attributes.socialLinks).length > 2
    )
      score += 3;
