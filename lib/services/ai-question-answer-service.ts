...
export class AIQuestionAnswerService {
  ...
  private categorizeQuestion(question: string): QuestionCategory {
    ...
    // أنماط للتصنيف
    const patterns = {
      hours: /ساعات|مفتوح|مغلق|يفتح|يغلق|opening hours|open|close|hours|working hours|متى تفتح|متى تغلق/i,
...
- Include the business name in the answer if possible`;
  ...
  private calculateConfidence(
    ...
    const hasSpecificInfo = /\d|street|st\.|avenue|ave\.|road|rd\.|شارع|طريق/.test(answer.toLowerCase());
    if (hasSpecificInfo) {
      confidence += 10;
    }
    ...
    const hasApology = /sorry|unfortunately|don't have|لا نملك|عذراً|للأسف/.test(answer.toLowerCase());
    if (hasApology) {
      confidence -= 20;
    }
