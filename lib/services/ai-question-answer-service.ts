...
export class AIQuestionAnswerService {
  async generateAnswer(context: QuestionContext): Promise<AnswerResult> {
    ...
    try {
      ...
      throw new Error(
        err.message ||
          "Failed to generate answer. Please check your AI configuration.",
      );
    }
  }
  ...
  private categorizeQuestion(question: string): QuestionCategory {
    ...
    const patterns = {
      ...
      services:
        /خدمات|تقدم|توفر|services|what do you|offer|provide|do you have/i,
...
- Include the business name in the answer if possible`;
  ...
  private calculateConfidence(
    ...
    const hasSpecificInfo =
      /\d|street|st\.|avenue|ave\.|road|rd\.|شارع|طريق/.test(
        answer.toLowerCase(),
      );
    if (hasSpecificInfo) {
      confidence += 10;
    ...
    const hasApology =
      /sorry|unfortunately|don't have|لا نملك|عذراً|للأسف/.test(
        answer.toLowerCase(),
      );
    if (hasApology) {
      confidence -= 20;
