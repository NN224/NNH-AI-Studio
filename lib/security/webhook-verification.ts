...
export function verifyGoogleWebhookSignature(
  ...
  if (!signature) {
    console.warn('[Webhook Security] Missing X-Goog-Signature header');
    return false;
  }

  if (!secret) {
    console.error('[Webhook Security] Missing webhook secret');
    return false;
  }
  ...
  try {
    ...
    if (!timestamp || !receivedSignature) {
      console.warn('[Webhook Security] Invalid signature format - expected "timestamp.signature"');
      return false;
    }
    ...
    if (isNaN(messageTime)) {
      console.warn('[Webhook Security] Invalid timestamp in signature');
      return false;
    }
    ...
    if (timeDiff > 300) {
      ...
      console.warn(
        '[Webhook Security] Signature timestamp too old:',
        timeDiff,
        'seconds (max: 300)'
      );
      return false;
    ...
    if (!isValid) {
      console.warn('[Webhook Security] Signature verification failed');
    }
...
export function validateWebhookPayload(payload: any): boolean {
  if (!payload || typeof payload !== 'object') {
    console.warn('[Webhook Validation] Payload is not an object');
    return false;
  }

  // Required field: name (resource name like "accounts/{accountId}/locations/{locationId}")
  if (!payload.name || typeof payload.name !== 'string') {
    console.warn('[Webhook Validation] Missing or invalid "name" field');
    return false;
  }
  ...
  if (!payload.notificationTypes || !Array.isArray(payload.notificationTypes)) {
    console.warn('[Webhook Validation] Missing or invalid "notificationTypes" field');
    return false;
  }

  if (payload.notificationTypes.length === 0) {
    console.warn('[Webhook Validation] Empty notificationTypes array');
    return false;
  }
  ...
  if (!hasValidType) {
    console.warn(
      '[Webhook Validation] Invalid or unknown notification types:',
  ...
  if (!resourcePattern.test(payload.name)) {
    console.warn(
      '[Webhook Validation] Resource name does not match expected pattern:',
      payload.name
    );
...
export function extractLocationInfo(payload: any): {
  ...
  if (!match) {
    console.warn('[Webhook] Could not extract location info from:', payload.name);
    return null;
  }
...
export class WebhookSecurityError extends Error {
  ...
  constructor(code: WebhookSecurityErrorCode, message: string, statusCode: number = 401) {
    super(message);
