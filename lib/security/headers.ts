 ...
 * In non-production (development/test), returns a permissive policy for Playwright/HMR
 * In production, returns a strict security policy
 */
export function getCSPDirectives(): Record<string, string[]> | null {
  // In development/test mode, return null to skip CSP entirely
  // This is required for Playwright testing and HMR to work properly
  if (!isProduction) {
    return null;
  }
 ...
 * Returns null in development/test mode to skip CSP entirely
 */
export function generateCSP(): string | null {
  const directives = getCSPDirectives();

  // Skip CSP in development/test mode
  if (!directives) {
    return null;
  }
...
export function getCORSHeaders(origin?: string): Record<string, string> {
  ...
  return {
    "Access-Control-Allow-Origin":
      origin && isOriginAllowed(origin) ? origin : allowedOrigin,
