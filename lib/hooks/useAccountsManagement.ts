...
export function useAccountsManagement() {
  ...
  const fetchAccounts = useCallback(async (): Promise<GmbAccount[]> => {
    // تحديد نوع الإرجاع
    setLoading(true);
    ...
    try {
      ...
      console.log(
        `[useAccountsManagement] Found ${accountsData?.length ?? 0} accounts in DB.`,
      );
      ...
      const validAccounts = accountsWithLocations.filter(
        (acc) => acc.id,
      ) as GmbAccount[];
  ...
  const handleSync = useCallback(
    async (accountId: string, isAutoSync = false) => {
      setSyncing(accountId);
      console.log(
        `[useAccountsManagement] ${isAutoSync ? "Auto-syncing" : "Syncing"} account ${accountId}`,
      );
      try {
        // ✅ MODERNIZED: Use queue-based sync instead of direct sync
        const response = await fetch("/api/gmb/enqueue-sync", {
          ...
          body: JSON.stringify({
            accountId,
            syncType: "full",
            priority: isAutoSync ? 3 : 8, // Higher priority for manual syncs
          }),
        });
        ...
        console.log("[useAccountsManagement] Sync enqueued:", data);
        toast({
          title: isAutoSync ? "Auto-Sync Queued!" : "Sync Queued Successfully!",
          description:
  ...
  const handleDisconnect = useCallback(
    async (accountId: string) => {
      ...
      if (
        !confirm(
          "هل أنت متأكد أنك تريد فصل هذا الحساب؟ ستتوقف المزامنة ولكن لن يتم حذف البيانات الحالية.",
        )
      )
        return;
