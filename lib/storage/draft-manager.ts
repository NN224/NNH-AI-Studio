...
type DraftTelemetryEvent = 'draft_saved' | 'draft_deleted' | 'draft_restored' | 'draft_imported' | 'draft_exported'
...
function telemetry(event: DraftTelemetryEvent, payload?: Record<string, unknown>) {
  if (process.env.NODE_ENV !== 'production') {
    console.warn(`[DraftTelemetry] ${event}`, payload ?? {})
  }
  if (isBrowser) {
    try {
...
export function saveDraft(key: string, type: DraftKind, content: string, metadata?: Record<string, unknown>) {
  const storage = getStorage()
...
export function cleanupDrafts() {
  ...
  Object.keys(storage).forEach((key) => {
    ...
    try {
      ...
      if (now - parsed.updatedAt > DRAFT_EXPIRY_MS || parsed.version !== DRAFT_VERSION) {
        storage.removeItem(key)
...
export function importDrafts(records: DraftRecord[], { merge = true }: { merge?: boolean } = {}) {
  const storage = getStorage()
  ...
  records.forEach((draft) => {
    ...
    storage.setItem(storageKey(draft.key), JSON.stringify({ ...draft, version: DRAFT_VERSION }))
  })
...
export function recordDraftTelemetry(event: DraftTelemetryEvent, payload?: Record<string, unknown>) {
