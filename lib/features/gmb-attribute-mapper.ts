 ...
 */
export function mapGMBAttributeToFeatureKey(
  gmbAttributeName: string,
): string | null {
  // Direct mapping
 ...
 */
export function extractFeatureKeysFromGMBAttributes(
  ...
  for (const attr of attributes) {
    ...
    if (attrName === "attributes/wi_fi") {
      const wifiValue = Array.isArray(attr.values)
        ? attr.values[0]
        : attr.values;
