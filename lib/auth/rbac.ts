...
export function checkPermission(role: TeamRole | null | undefined, resource: Resource, action: string) {
...
export function getRequestTeamContext() {
  ...
  const cookieRole = cookieStore.get('nnh-team-role')?.value as TeamRole | undefined
  return {
...
export async function fetchMembershipRole(
  ...
  if (error) {
    console.warn('[RBAC] Failed to fetch membership role', error)
    return null
  }
...
export async function assertPermission(
  supabase: SupabaseClient,
  options: { userId?: string | null; teamId?: string | null; resource: Resource; action: string }
) {
...
export async function getTeamsForUser(supabase: SupabaseClient, userId: string) {
  const { data, error } = await supabase
    .from('team_members')
    .select(
      `
  ...
  if (error) {
    console.error('[RBAC] Failed to fetch teams', error)
    return []
  }
...
export async function inviteTeamMember(
  ...
  if (error) {
    console.error('[RBAC] Failed to create invitation', error)
...
export async function updateMemberRole(
  ...
  const { data, error } = await supabase
    ...
    .select('team_id')
  if (error) {
    console.error('[RBAC] Failed to update member role', error)
    throw new Error(error.message)
  }
  return data
}
...
export async function removeMember(
  ...
  const { error } = await supabase
    ...
    .eq('user_id', targetUserId)
  if (error) {
    console.error('[RBAC] Failed to remove member', error)
    throw new Error(error.message)
  }
}
