  ...
  if (typeof value === "string") {
    try {
      ...
      if (process.env.NODE_ENV !== "production") {
        console.warn(
          "[features/profile] Failed to parse string metadata",
          error,
        );
      }
    }
...
export async function GET(
  ...
  try {
    ...
    if (process.env.NODE_ENV !== "production") {
      console.error("[GET /api/features/profile/:locationId] Error", {
        message,
        status,
        error,
      });
...
export async function PUT(
  ...
  try {
    ...
    const status = message === "Location not found" ? 404 : 500;
    if (process.env.NODE_ENV !== "production") {
      console.error("[PUT /api/features/profile/:locationId] Error", {
        message,
        status,
        error,
      });
