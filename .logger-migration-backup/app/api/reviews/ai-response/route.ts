...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (!aiProvider) {
        console.error(
          "[AI Response] No AI provider configured for user:",
          user.id,
        );
        return NextResponse.json(
          {
...
Generate a natural, professional, short reply (max 500 chars). Tone: friendly and authentic.`;
      ...
      const { content: generatedResponse, usage } =
        await aiProvider.generateCompletion(
          prompt,
          "review_auto_reply",
          review.location_id,
        );
      ...
      const qualityScore =
        Math.min(
          100,
          generatedResponse.length / 5 + (sentiment === "positive" ? 10 : 0),
        ) || 70;
          ...
          Math.min(
            ...
            Math.max(
              ...
              70 +
                ...
                (generatedResponse.length > 100 &&
                generatedResponse.length < 400
                  ? 10
                  : 0),
            ),
          ),
