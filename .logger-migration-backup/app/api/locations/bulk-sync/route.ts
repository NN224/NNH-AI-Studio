  ...
  try {
    ...
    do {
      ...
      if (reviews.length > 0) {
        ...
        if (reviewsError) {
          console.error(
            `[Bulk Sync] Error upserting reviews for location ${location.id}:`,
            reviewsError,
          );
        } else {
    ...
    do {
      ...
      if (media.length > 0) {
        ...
        if (mediaError) {
          console.error(
            `[Bulk Sync] Error upserting media for location ${location.id}:`,
            mediaError,
          );
        } else {
    ...
    console.error(`[Bulk Sync] Error syncing location ${location.id}:`, err);
  ...
  try {
    ...
    for (const [accountId, locations] of locationsByAccount.entries()) {
      ...
      try {
        ...
        const err = error instanceof Error ? error : new Error(String(error));
        console.error(
          `[Bulk Sync] Error getting access token for account ${accountId}:`,
          err,
        );
