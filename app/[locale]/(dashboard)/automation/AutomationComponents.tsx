...
export function AutomationStatsCard({
  ...
  return (
    <div
      className={`bg-zinc-900/50 border rounded-xl p-6 ${colorClasses[color]}`}
    >
...
export function AutomationLocationCard({
  settings,
  logs,
}: AutomationLocationCardProps) {
  ...
  const modules = useMemo(
    () => [
      ...
      {
        label: t("smartPosting"),
        enabled: settings.smartPostingEnabled,
        icon: "📝",
      },
      {
        label: t("competitorMonitor"),
        enabled: settings.competitorMonitoringEnabled,
        icon: "📈",
      },
      {
        label: t("insightsReports"),
        enabled: settings.insightsReportsEnabled,
        icon: "📊",
      },
    ],
    [settings, t],
        ...
        {isExpanded && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-zinc-800/30 rounded-lg p-4 space-y-2 text-xs text-zinc-400">
              ...
              <p>
                {t("status")}{" "}
                {settings.autoReplyEnabled ? t("enabled") : t("disabled")}
              </p>
              <p>
                {t("minRating")} {settings.autoReplyMinRating ?? t("notSet")}
              </p>
              <p>
                {t("tone")} {settings.replyTone ?? t("default")}
              </p>
            </div>
            <div className="bg-zinc-800/30 rounded-lg p-4 space-y-2 text-xs text-zinc-400">
              <h4 className="font-medium text-white flex items-center gap-2 text-sm">
              ...
              <p>
                {t("status")}{" "}
                {settings.smartPostingEnabled ? t("enabled") : t("disabled")}
              </p>
              <p>
                {t("frequency")} {settings.postFrequency ?? t("notSet")}
              </p>
            </div>
            <div className="bg-zinc-800/30 rounded-lg p-4 space-y-2 text-xs text-zinc-400">
              <h4 className="font-medium text-white flex items-center gap-2 text-sm">
              ...
              <p>
                ...
                {settings.competitorMonitoringEnabled
                  ? t("enabled")
                  : t("disabled")}
              </p>
              <p>
              ...
              <p>
                {t("reportCadence")} {settings.reportFrequency ?? "weekly"}
              </p>
            </div>
            <div className="bg-zinc-800/30 rounded-lg p-4 space-y-2 text-xs text-zinc-400">
              <h4 className="font-medium text-white flex items-center gap-2 text-sm">
...
export function ActivityLog({
  logs,
}: Readonly<{ logs: ReadonlyArray<AutomationLogEntry> }>) {
      ...
      <div className="space-y-2">
        {filteredLogs.length === 0 ? (
          ...
          filteredLogs.map((log) => (
            ...
            >
              <div
                className={`text-xl ${log.status === "success" ? "text-green-400" : "text-red-400"}`}
              >
                {log.status === "success" ? "✓" : "✗"}
              </div>
