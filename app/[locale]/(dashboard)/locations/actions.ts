...
async function withRetry<T>(
  ...
  for (let i = 0; i < MAX_RETRIES; i++) {
    try {
      ...
      gmbLogger.error(
        `[${actionName}] Completed in ${duration}ms after ${i + 1} attempt(s).`,
      );
      return result;
    } catch (err) {
      lastError = err;
      gmbLogger.error(
        `[${actionName}] Attempt ${i + 1} failed`,
        err instanceof Error ? err : new Error(String(err)),
      );
      await new Promise((res) => setTimeout(res, 500 * (i + 1))); // exponential backoff
    }
  }
  gmbLogger.error(`[${actionName}] All attempts failed.`);
  throw lastError;
}
...
async function logAction(action: string, status: string, details?: unknown) {
  ...
  try {
    ...
    gmbLogger.error(
      "[LocationLogs] Failed to record action log",
      err instanceof Error ? err : new Error(String(err)),
    );
  }
}
  ...
  try {
    const result = await withRetry(async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/locations/bulk-sync`,
        {
          method: "POST",
          cache: "no-store",
        },
