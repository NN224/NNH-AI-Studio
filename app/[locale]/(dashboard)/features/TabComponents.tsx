...
export function BusinessInfoTab({
  ...
  const handleInputChange = (
    field: keyof Pick<
      BusinessProfilePayload,
      "locationName" | "shortDescription" | "phone" | "website"
    >,
    value: string,
  ) => {
    if (disabled) return;
  ...
  const aiSuggestions = useMemo(() => {
    ...
    if (!amenityFeatures.has("wheelchair_accessible")) {
      suggestions.push({
        ...
        description:
          "This attribute appears in accessibility filters and improves visibility",
        onApply: () => {
          const nextFeatures = withUpdatedFeatures(
            profile,
            "amenities",
            (current) => [...current, "wheelchair_accessible"],
          );
    ...
    if (!description.includes("near me") && description.length > 50) {
      suggestions.push({
        ...
        onApply: () => {
          onChange({
            ...profile,
            description: `${profile.description} Find us near me in Dubai.`,
          });
          onDirty();
    ...
    if (profile.additionalCategories.length === 0 && profile.primaryCategory) {
      suggestions.push({
        ...
        onApply: () => {
          toast.info(
            "Please add additional categories from the Categories section above",
          );
        },
      });
    }
  ...
  const addCategory = (category: string) => {
    if (disabled) return;
    if (
      profile.additionalCategories.length >= 9 ||
  ...
  const removeCategory = (category: string) => {
    ...
    onChange({
      ...profile,
      additionalCategories: profile.additionalCategories.filter(
        (item) => item !== category,
      ),
    });
    onDirty();
  ...
  const handleLinkChange = (
    key: keyof typeof profile.specialLinks,
    value: string,
  ) => {
    if (disabled) return;
    onChange({
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      {aiSuggestions.length > 0 && (
        <div className="bg-linear-to-r from-purple-950/30 to-orange-950/30 border border-purple-500/30 rounded-xl p-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-zinc-400">
                  Smart suggestions to improve your profile
                </p>
              </div>
            </div>
            <Button
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {COMMON_CATEGORIES.filter(
                (cat) =>
                  cat !== profile.primaryCategory &&
                  !(profile.additionalCategories || []).includes(cat),
              )
                .slice(0, 6)
                .map((cat) => (
                  <Button
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
        ...
        <p className="text-sm text-zinc-400 mb-4">
          Connect your social media profiles (from Google My Business
          attributes)
        </p>
        <div className="grid gap-4 sm:grid-cols-2">
          {[
            ...
            <div key={social.key}>
              ...
              <input
                ...
                onChange={(e) =>
                  handleSocialLinkChange(social.key, e.target.value)
                }
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 p-4 rounded-lg border border-zinc-800 bg-zinc-950/50">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-1">
                Enable if you provide services in a geographic area rather than
                at a fixed location
              </p>
            </div>
          </div>
...
export function FeaturesTab({
  ...
  const activeFeatureCount = useMemo(() => {
    return FEATURE_CATEGORY_KEYS.reduce(
      (acc, key) => acc + (profile.features?.[key]?.length ?? 0),
      0,
    );
  }, [profile.features]);
                  ...
                  {features.map((feature) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          toggleFeature(category, feature.key, !isActive)
                        }
                        disabled={disabled}
                        className={cn(
