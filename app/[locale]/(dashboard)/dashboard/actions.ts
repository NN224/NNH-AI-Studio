...
export async function disconnectLocation(
  locationId: string,
): Promise<DisconnectLocationResult> {
  ...
  try {
    ...
    const location = await fetchLocationForUser(
      supabase,
      adminClient,
      locationId,
      user.id,
    ...
    const accountId =
      (location as any).account_id || (location as any).gmb_accounts?.id;
    ...
    const { disconnectGMBAccount } = await import(
      "@/server/actions/gmb-account"
    );
    ...
    if (result.success) {
      return {
        success: true,
        message:
          result.message || "Location and account disconnected successfully",
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Unexpected disconnect error occurred.",
    };
  }
}
...
export async function syncLocation(locationId: string) {
  ...
  try {
    ...
    try {
      ...
      console.error(
        "[Dashboard Actions] Token decryption failed for account:",
        account.id,
      );
    ...
    const needsRefresh =
      !accessToken || !expiresAt || expiresAt - bufferMs <= now;
    ...
    if (needsRefresh) {
      ...
      try {
        ...
        newExpiresAt.setSeconds(
          newExpiresAt.getSeconds() + (tokens.expires_in || 3600),
        );
        ...
        if (tokenUpdateError) {
          console.error(
            "[syncLocation] Failed to save refreshed token",
            tokenUpdateError,
          );
        }
      } catch (tokenError) {
        ...
        if (
          ...
          return {
            success: false,
            error:
              "Your Google account connection has expired. Please go to Settings and reconnect your Google My Business account.",
          };
        }
    ...
    // Old sync removed - use global sync system (gmb-sync-v2)
    return {
      success: false,
      error: "Please use the global sync button in the header",
    };
  } catch (error) {
...
export async function generateWeeklyTasks(locationId: string) {
  ...
  try {
    ...
    const pendingReviews =
      reviews?.filter((r) => !r.review_reply || r.review_reply.trim() === "")
        .length || 0;
    const unansweredQuestions = questions?.length || 0;
    const recentNegativeReviews =
      reviews?.filter((r) => r.rating && r.rating <= 2 && !r.review_reply)
        .length || 0;
    ...
    if (tasks.length === 0) {
      tasks.push({
        ...
        description:
          "Your location is performing well. Continue monitoring reviews and responding promptly.",
...
export async function getDashboardDataWithFilter(
  ...
  try {
    ...
    if (!user) {
      return {
        success: true,
        data: { reviews: [], locations: [], questions: [] },
      };
    }
...
export async function getDashboardStats() {
  ...
  // Fetch main stats from materialized view using RPC
  ...
  // Use response rate from RPC (response_rate_percent) or fallback to 0
  const responseRate = stats?.response_rate_percent || 0;

  // Provide fallback values when no stats are available (new user)
  const safeStats = {
    total_reviews: stats?.reviews_count || 0,
    avg_rating: stats?.average_rating || 0,
    pending_reviews:
      (stats?.reviews_count || 0) - (stats?.replied_reviews_count || 0),
    pending_questions: 0, // Not in RPC, need separate query
    replied_reviews: stats?.replied_reviews_count || 0,
  ...
  const reviewsTrend =
    previousReviews && previousReviews > 0
      ? Math.round(
          (((recentReviews || 0) - previousReviews) / previousReviews) * 100,
        )
      : 0;
