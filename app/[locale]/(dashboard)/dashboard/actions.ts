...
async function fetchLocationForUser(
  ...
  const ownerId =
    (adminLocation as any).user_id ?? (adminLocation as any).gmb_accounts?.user_id ?? null
  ...
  try {
    ...
    if (needsRefresh) {
      ...
      try {
        ...
        const errorMessage =
          tokenError instanceof Error ? tokenError.message : 'Failed to refresh Google access token'
...
export async function generateWeeklyTasks(locationId: string) {
  ...
  try {
    ...
    const pendingReviews =
      reviews?.filter((r) => !r.review_reply || r.review_reply.trim() === '').length || 0
    const unansweredQuestions = questions?.length || 0
    const recentNegativeReviews =
      reviews?.filter((r) => r.rating && r.rating <= 2 && !r.review_reply).length || 0
...
export async function getDashboardDataWithFilter(startDate?: string, endDate?: string) {
  const supabase = await createClient()
...
export async function getDashboardStats() {
  ...
  const { data: stats, error: statsError } = await supabase
    ...
    .select(
      'total_reviews, avg_rating, pending_reviews, pending_questions, replied_reviews, calculated_response_rate',
    )
    .eq('user_id', user.id)
    .maybeSingle()
