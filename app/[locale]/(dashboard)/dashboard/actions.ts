...
// Import types
import type { DashboardDataResult, LocationWithGMBAccount, RefreshTokenActionResult } from './types'
...
export async function getDashboardData(): Promise<DashboardDataResult> {
  try {
    ...
    throw error instanceof DashboardServiceError
      ? error
      : new DashboardServiceError('Failed to fetch dashboard data', 'FETCH_ERROR')
  }
}
...
export async function refreshTokenAction(
  ...
  try {
    ...
    const location = await locationService.getLocationWithAccount(locationId, user.id)
 ...
 */
export async function refreshAllExpiredTokens(): Promise<RefreshTokenActionResult[]> {
  try {
    const supabase = await createClient()
    const {
      data: { user },
...
export async function updateLocationMetadata(
  ...
  try {
    ...
    // Verify ownership
    const isOwner = await locationService.isLocationOwnedByUser(locationId, user.id)
    if (!isOwner) {
      return {
        success: false,
        message: 'Location not found or access denied',
