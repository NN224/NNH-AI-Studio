...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalQuestionsToday = (questionsToday || 0) + (questionsTodayAlt || 0);
    ...
    const autoReplyEnabled = settings?.enabled || accountSettingsData.autoReplyEnabled || false;
    ...
    console.error('[AutoPilot Stats] Error:', error);
    return NextResponse.json(
      { 
        success: false, 
        error: error instanceof Error ? error.message : 'Failed to fetch autopilot stats' 
      },
      { status: 500 }
    );
  }
