...
async function publishAnswerToGoogle(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.error?.message || errorData.message || 'Failed to publish answer to Google',
    )
  }
...
async function deleteAnswerFromGoogle(
  ...
  if (!response.ok && response.status !== 404) {
    ...
    throw new Error(
      errorData.error?.message || errorData.message || 'Failed to delete answer from Google',
    )
  }
}
...
export async function POST(request: NextRequest, { params }: { params: { questionId: string } }) {
  try {
    ...
    // Rate limiting
    const { success: rateLimitSuccess, headers: rateLimitHeaders } = await checkRateLimit(user.id)
    if (!rateLimitSuccess) {
...
export async function DELETE(request: NextRequest, { params }: { params: { questionId: string } }) {
  try {
    ...
    if (question.external_question_id && locationDel.location_id && question.answer_text) {
      try {
        const accessToken = await getValidAccessToken(supabase, account.id)
        const metadata = (question.metadata as Record<string, unknown>) || {}
        const googleAnswer = metadata.google_answer as { name?: string } | undefined
    ...
    try {
      ...
      console.warn('[Activity Log] Failed to log activity:', logError)
    }
