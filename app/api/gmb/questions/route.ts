...
export async function GET(request: NextRequest) {
  try {
    ...
    if (accountsError) {
      console.error(
        "[Questions API] Error fetching active accounts:",
        accountsError,
      );
    ...
    if (locationsError) {
      console.error(
        "[Questions API] Error fetching active locations:",
        locationsError,
      );
      return errorResponse(
        "DATABASE_ERROR",
    ...
    if (questionsError) {
      console.error(
        "[Questions API] Error fetching questions:",
        questionsError,
      );
    ...
    const [pendingResult, answeredResult, draftResult, totalResult] =
    ...
    const pendingCount =
      pendingResult.status === "fulfilled" && pendingResult.value.count !== null
        ...
        : 0;
    const answeredCount =
      answeredResult.status === "fulfilled" &&
      answeredResult.value.count !== null
    ...
    const sanitizedQuestions = (questions || []).map((question) => ({
      ...
      answer_text: question.answer_text
        ? sanitizeHtml(question.answer_text)
