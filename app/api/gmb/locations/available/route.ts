...
// Google API URLs
const GMB_ACCOUNTS_API = 'https://mybusinessbusinessinformation.googleapis.com/v1'
...
export const POST = withSecureApi<AvailableLocationsBody>(
  async (_request, { body, user }) => {
    ...
    if (gmbAccount.user_id !== user.id) {
      throw new ApiError(ErrorCode.FORBIDDEN, 'You do not have access to this account', 403)
    }
    ...
    try {
      ...
      throw new ApiError(ErrorCode.INTERNAL_ERROR, 'Failed to decrypt access token', 500)
    }
    ...
    const importedLocationIds = new Set((importedLocations || []).map((l) => l.location_id))
