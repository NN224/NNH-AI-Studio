...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingJob) {
      console.log(`[Enqueue Sync] Job already exists for account ${accountId}:`, existingJob.id);
      return NextResponse.json({
    ...
    console.log(`[Enqueue Sync] ✅ Enqueued ${syncType} sync for account ${account.account_name} (${accountId})`);
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: jobs, error: queueError } = await supabase
      .from('sync_queue')
      .select('id, account_id, sync_type, status, attempts, max_attempts, created_at, scheduled_at')
      .eq('user_id', user.id)
      .order('created_at', { ascending: false })
      .limit(50);
