...
export async function GET() {
  try {
    ...
    if (!err4 && accounts) {
      const settingsIssues = accounts.filter(
        (a) =>
    ...
    if (
      auditResults.some(
        (r) => r.severity === "CRITICAL" && r.category === "Authentication",
      )
    ) {
      recommendations.push(
        "Immediate action required: Re-authenticate GMB accounts with authentication issues",
      );
    }

    if (auditResults.some((r) => r.issue.includes("stale data"))) {
      recommendations.push(
        "Consider enabling auto-sync for accounts with stale data",
      );
    }

    if (auditResults.some((r) => r.issue.includes("default settings"))) {
      recommendations.push(
        "Review and configure settings for optimal performance",
      );
    }
    ...
    const summary = {
      ...
      critical_count: auditResults.filter((r) => r.severity === "CRITICAL")
        .length,
      warning_count: auditResults.filter((r) => r.severity === "WARNING")
        .length,
