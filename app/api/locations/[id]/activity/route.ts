...
export async function GET(
  ...
  try {
    ...
    if (postsError) {
      apiLogger.error("Error fetching posts", new Error(String(postsError)), {
        locationId,
      });
    }
    ...
    if (mediaError) {
      apiLogger.error("Error fetching media", new Error(String(mediaError)), {
        locationId,
      });
    }
    ...
    if (questionsError) {
      apiLogger.error(
        ...
        new Error(String(questionsError)),
        { locationId },
      );
    }
    ...
    apiLogger.error("Unexpected error in location activity", err);
    return NextResponse.json(
      {
