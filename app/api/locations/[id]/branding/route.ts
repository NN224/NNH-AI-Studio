...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const variantEntry = (formData.get('variant') as BrandingVariant | null) ?? 'cover'
    ...
    if (uploadError) {
      console.error('[Branding Upload] Upload failed', uploadError)
    ...
    if (locationError) {
      console.error('[Branding Upload] Failed to fetch location', locationError)
    ...
    if (updateError) {
      console.error('[Branding Upload] Failed to update metadata', updateError)
      return NextResponse.json({ error: 'Failed to persist branding image' }, { status: 500 })
    }
...
function buildStoragePath(userId: string, locationId: string, variant: BrandingVariant) {
...
function normalizeMetadata(metadata: unknown): Record<string, any> {
  ...
  if (typeof metadata === 'string') {
    try {
      ...
      console.warn('[Branding Upload] Failed to parse metadata string', error)
      return {}
    }
  }
