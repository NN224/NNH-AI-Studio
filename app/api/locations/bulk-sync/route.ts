...
interface GMBReview {
  ...
  createTime?: string;
  updateTime?: string;
  reviewReply?: { updateTime?: string } | null;
}

interface GMBMediaItem {
  name: string;
  mediaFormat?: string;
  sourceUrl?: string;
  googleUrl?: string;
...
async function fetchReviews(
  ...
  const url = new URL(
    `${GMB_CONSTANTS.GBP_LOC_BASE}/${locationResource}/reviews`,
  );
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to fetch reviews: ${errorData.error?.message || "Unknown error"}`,
    );
  }
...
async function fetchMedia(
  ...
  accountResource: string,
  pageToken?: string,
): Promise<{ media: GMBMediaItem[]; nextPageToken?: string }> {
  const url = new URL(
    `${GMB_CONSTANTS.GBP_LOC_BASE}/${locationResource}/media`,
  );
  url.searchParams.set("pageSize", "50");
  if (pageToken) {
    url.searchParams.set("pageToken", pageToken);
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    throw new Error(
      `Failed to fetch media: ${errorData.error?.message || "Unknown error"}`,
    );
  }
...
async function syncLocation(
  ...
  try {
    ...
    do {
      ...
      if (reviews.length > 0) {
        const reviewRows = reviews.map((review: GMBReview) => ({
        ...
        if (reviewsError) {
          apiLogger.error(
            "[Bulk Sync] Error upserting reviews for location",
    ...
    do {
      ...
      if (media.length > 0) {
        const mediaRows = media.map((item: GMBMediaItem) => ({
          location_id: location.id,
          media_id: item.name,
        ...
        if (mediaError) {
          apiLogger.error(
            "[Bulk Sync] Error upserting media for location",
            mediaError instanceof Error
              ? mediaError
              : new Error(String(mediaError)),
            { locationId: location.id },
          );
        } else {
    ...
    apiLogger.error("[Bulk Sync] Error syncing location", err, {
      locationId: location.id,
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !locationIds.every((id) => typeof id === "string" && uuidRegex.test(id))
    ) {
      return NextResponse.json(
        {
    ...
    if (locationsError) {
      apiLogger.error(
        "[POST /api/locations/bulk-sync] DB Error",
        locationsError instanceof Error
          ? locationsError
          : new Error(String(locationsError)),
        {
          code: (locationsError as any)?.code,
          userId: user.id,
    ...
    for (const [accountId, locations] of locationsByAccount.entries()) {
      ...
      try {
        ...
        const err = error instanceof Error ? error : new Error(String(error));
        apiLogger.error(
          "[Bulk Sync] Error getting access token for account",
          err,
          { accountId },
        );
    ...
    const err = error instanceof Error ? error : new Error(String(error));
    apiLogger.error("[POST /api/locations/bulk-sync] Unexpected error", err, {
      timestamp: new Date().toISOString(),
    });
