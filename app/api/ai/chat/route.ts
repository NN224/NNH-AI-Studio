 ...
 */
async function fetchDashboardContext(userId: string, supabase: SupabaseClient) {
  ...
  return {
    ...
    locations: locations as LocationData[] | null,
    recentReviews: recentReviews as ReviewData[] | null,
    pendingReviews: pendingReviews as ReviewData[] | null,
  }
}
...
interface DashboardContext {
  ...
  pendingReviews: ReviewData[] | null
}
...
- Average recent rating: ${
    recentReviews?.length
      ? (
          recentReviews.reduce((sum: number, r: ReviewData) => sum + r.rating, 0) /
          recentReviews.length
        ).toFixed(2)
      : 'N/A'
  }
