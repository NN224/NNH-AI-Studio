...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      reviewsLogger.error(
        "Database error fetching reviews for analyze-sentiment",
    ...
    const updatePromises = Array.from(mlResults.entries()).map(
      async ([reviewId, result]) => {
        ...
        if (updateError) {
          reviewsLogger.error(
            "Failed to update review with sentiment",
    ...
    if (successCount > 10) {
      try {
        ...
        reviewsLogger.error(
          "Failed to refresh sentiment summary",
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [{ count: totalReviews }, { count: analyzedReviews }] =
      await Promise.all([
        supabase
          .from("gmb_reviews")
    ...
    const percentComplete = totalReviews
      ? Math.round(((analyzedReviews || 0) / totalReviews) * 100)
      : 0;
    ...
    reviewsLogger.error(
      "Status check error (analyze-sentiment)",
      error instanceof Error ? error : new Error(String(error)),
    );
    return NextResponse.json(
      { error: "Internal server error" },
