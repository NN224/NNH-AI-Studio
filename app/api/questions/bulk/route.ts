    ...
    switch (action) {
      case 'analyze': {
        ...
        const businessContext = {
          businessName: questions[0].gmb_locations?.[0]?.location_name || 'Unknown Business'
        };
        ...
        for (const [questionId, analysis] of analyses) {
          const confidence = mlQuestionsService.calculateConfidence(analysis, businessContext);
        ...
        for (const question of questions) {
          ...
          if (options?.autoAnswer && !answerText) {
            const analysis = await mlQuestionsService.analyzeQuestion(
              ...
              { businessName: question.gmb_locations?.[0]?.location_name || 'Unknown Business' }
            );
            answerText = analysis.suggestedAnswer;
          }

          if (!answerText) {
            results.failed.push({ id: question.id, error: 'No answer provided' });
            continue;
          }
      ...
      case 'approve': {
        ...
        if (error) {
          results.failed = questionIds.map(id => ({ id, error: error.message }));
        } else {
