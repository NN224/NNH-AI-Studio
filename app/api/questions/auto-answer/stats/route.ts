  ...
  try {
    ...
    if (error) {
      questionsLogger.error(
        "Error fetching auto-answer logs",
    ...
    const successfulAnswers = logs.filter(
      (log) => log.status === "posted",
    ).length;
    const failedAnswers = logs.filter((log) => log.status === "failed").length;
    const pendingAnswers = logs.filter(
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      weeklyData.push({
        ...
        avgConfidence:
          dayTotal > 0
            ? Math.round(
                dayLogs.reduce((sum, l) => sum + (l.confidence_score || 0), 0) /
                  dayTotal,
              )
            : 0,
      });
