...
export async function GET() {
  try {
    ...
    const latestStatus =
      syncStatus && syncStatus.length > 0 ? syncStatus[0] : null;
    ...
    const recentJobsLastHour =
      recentJobs?.filter((job) => new Date(job.created_at) > oneHourAgo) || [];
