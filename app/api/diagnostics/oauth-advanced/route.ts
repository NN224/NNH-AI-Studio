...
export async function GET() {
  ...
  try {
    ...
    if (accountsError) {
      ...
      return Response.json({ success: false, logs, error: accountsError }, { status: 500 })
    }
    ...
    log(`✅ Found GMB Account: ${account.account_name} (${account.account_id})`)
    log(`   ID: ${account.id}`)
    ...
    try {
      ...
      if (!account.refresh_token) throw new Error('Missing refresh_token in DB')
      ...
      log('❌ Token decryption failed', err.message)
      return Response.json({ success: false, logs, error: err.message }, { status: 500 })
    }
    ...
    try {
      ...
      const err = e instanceof Error ? e : new Error(String(e))
      log('❌ Refresh Token failed', err.message)
    ...
    try {
      ...
      if (expiresInMinutes < 0) {
        log('   (Using refreshed token for API call since stored one is expired)')
        const refreshResult = await refreshAccessToken(refreshToken!)
      ...
      const err = e instanceof Error ? e : new Error(String(e))
      log('❌ API Test failed', err.message)
      return Response.json({ success: false, logs, error: 'API call failed' }, { status: 500 })
    }
