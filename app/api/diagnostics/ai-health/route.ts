  ...
  try {
    ...
    if (aiHealth.providers.anthropic.configured) {
      try {
        ...
        // Never expose API key in client-visible endpoints
        ...
        aiHealth.providers.anthropic.tested = true
        aiHealth.providers.anthropic.error =
    ...
    if (aiRequests) {
      ...
      if (autoReplyRequests.length > 0) {
        ...
        aiHealth.features.auto_reply.success_rate = Math.round(
          (successful.length / autoReplyRequests.length) * 100,
        )
      ...
      if (autoAnswerRequests.length > 0) {
        ...
        aiHealth.features.auto_answer.success_rate = Math.round(
          (successful.length / autoAnswerRequests.length) * 100,
        )
      ...
      if (contentRequests.length > 0) {
        ...
        aiHealth.features.content_generation.success_rate = Math.round(
          (successful.length / contentRequests.length) * 100,
        )
