...
function rowToProgressEvent(
  row: SyncProgressRow | null,
): SyncProgressEvent | null {
  if (!row) return null;
  const totals =
    (row.total_locations ?? 0) +
      (row.total_reviews ?? 0) +
      (row.total_questions ?? 0) || 1;
  const completed =
    (row.synced_locations ?? 0) +
    (row.synced_reviews ?? 0) +
    (row.synced_questions ?? 0);
  const percentage = Math.max(
    0,
    Math.min(100, Math.round((completed / totals) * 100)),
  );
  ...
  return {
    ...
    status:
      row.status === "processing"
        ? "running"
        : (row.status as SyncProgressEvent["status"]),
    current: completed,
    total: totals,
    percentage,
    counts: {
    ...
    message:
      row.status === "failed"
        ? (row.error_message ?? "Sync failed")
        : undefined,
    error:
      row.status === "failed" ? (row.error_message ?? undefined) : undefined,
...
async function fetchLatestProgress(
  ...
  if (error && error.code !== "PGRST116") {
    syncLogger.warn("[Sync Progress SSE] Failed to fetch latest progress", {
      userId,
      accountId,
      syncId,
      error: String(error),
...
export async function GET(request: NextRequest) {
  ...
  const initial = await fetchLatestProgress(
    supabase,
    userId,
    accountId,
    syncId,
