    ...
    return {
      ...
      from: jest.fn(() => ({
        ...
        single: jest.fn().mockResolvedValue({
          data: { id: 'acc-uuid-1', user_id: 'user-1', account_id: 'accounts/123456789' },
          error: null,
        }),
      })),
    }
...
describe('GMB full sync flow (performTransactionalSync)', () => {
  beforeEach(() => {
      ...
      locations: [
        {
          ...
          metadata: { starRating: 4.6, totalReviewCount: 17, profileCompleteness: 0.8, status: 'verified' },
      ...
      reviews: [
        {
          ...
          reviewer: { displayName: 'Alice', profilePhotoUrl: 'https://img/alice' },
          starRating: 'STAR_FIVE',
          comment: 'Great service!',
          createTime: '2025-01-01T00:00:00Z',
          reviewReply: { comment: 'Thank you!', updateTime: '2025-01-02T00:00:00Z' },
      ...
      questions: [
        {
          ...
          author: { displayName: 'Bob', profilePhotoUrl: 'https://img/bob', type: 'CUSTOMER' },
    ...
    ;(global.fetch as jest.Mock).mockImplementation((input: RequestInfo | URL) => {
      ...
      if (url.startsWith('https://mybusinessbusinessinformation.googleapis.com/v1/accounts/123456789/locations')) {
        return Promise.resolve({ ok: true, json: async () => locationsResponse })
      }
      if (url.startsWith('https://mybusiness.googleapis.com/v4/accounts/123456789/locations/loc1/reviews')) {
        return Promise.resolve({ ok: true, json: async () => reviewsResponse })
      }
      if (url.startsWith('https://mybusinessqanda.googleapis.com/v1/accounts/123456789/locations/loc1/questions')) {
        return Promise.resolve({ ok: true, json: async () => questionsResponse })
      }
      return Promise.resolve({ ok: false, json: async () => ({ error: { message: 'not found' } }) })
    })
  })
  ...
  it('builds payload from Google endpoints and commits transactional sync', async () => {
    ...
    runSyncTransactionWithRetry.mockImplementation(async (_supabase: any, payload: SyncTransactionPayload) => {
      ...
      expect(rev.google_location_id).toBe('accounts/123456789/locations/loc1')
      // review shape keys
    ...
    // Act
    const result = await performTransactionalSync('acc-uuid-1', true /* includeQuestions */)
    ...
    expect(logAction).toHaveBeenCalledWith('sync', 'gmb_account', 'acc-uuid-1', expect.objectContaining({ status: 'success' }))
    expect(trackSyncResult).toHaveBeenCalledWith('user-1', true, expect.any(Number))

    // Verify fetch endpoints were hit
    expect(global.fetch).toHaveBeenCalledWith(
      expect.stringContaining('https://mybusinessbusinessinformation.googleapis.com/v1/accounts/123456789/locations'),
      expect.objectContaining({ headers: expect.objectContaining({ Authorization: expect.stringContaining('Bearer ') }) }),
    )
    expect(global.fetch).toHaveBeenCalledWith(
      expect.stringContaining('https://mybusiness.googleapis.com/v4/accounts/123456789/locations/loc1/reviews'),
      expect.any(Object),
    )
    expect(global.fetch).toHaveBeenCalledWith(
      expect.stringContaining('https://mybusinessqanda.googleapis.com/v1/accounts/123456789/locations/loc1/questions'),
      expect.any(Object),
    )
  })
})
