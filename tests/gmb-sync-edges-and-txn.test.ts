...
jest.mock('@/lib/gmb/helpers', () => ({
  ...
  buildLocationResourceName: (accountId: string, locationId: string) => `accounts/${accountId.replace(/^accounts\//,'')}/locations/${locationId.replace(/^(accounts\/[^/]+\/)?locations\//,'')}`,
  GMB_CONSTANTS: {
...
jest.mock('@/lib/supabase/transactions', () => ({
  runSyncTransactionWithRetry: (...args: any[]) => runSyncTransactionWithRetry(...args),
}))
...
describe('performTransactionalSync - transaction failure and mapping edges', () => {
  ...
  it('propagates transaction failure and does not refresh cache', async () => {
    ...
    const locationsResponse = { locations: [{ name: 'accounts/999/locations/l1', title: 'L1', openInfo: { status: 'OPEN' } }] }
    const reviewsResponse = { reviews: [] }
    const questionsResponse = { questions: [] }
    ;(global.fetch as jest.Mock).mockImplementation((input: RequestInfo | URL) => {
      const url = String(input)
      if (url.includes('/locations') && !url.includes('/reviews') && !url.includes('/questions')) {
        return Promise.resolve({ ok: true, json: async () => locationsResponse })
      }
      if (url.includes('/reviews')) return Promise.resolve({ ok: true, json: async () => reviewsResponse })
      if (url.includes('/questions')) return Promise.resolve({ ok: true, json: async () => questionsResponse })
      return Promise.resolve({ ok: true, json: async () => ({}) })
    })

    runSyncTransactionWithRetry.mockRejectedValueOnce(new Error('txn failed'))

    await expect(performTransactionalSync('acc-1', true)).rejects.toThrow('txn failed')

    // Ensure progress stopped with transaction error then complete error
    const pairs = publishSyncProgress.mock.calls.map((c) => `${c[0]?.stage}:${c[0]?.status}`)
    expect(pairs).toEqual(
      expect.arrayContaining(['transaction:running', 'transaction:error', 'complete:error']),
    )
    ...
    expect(logAction).toHaveBeenCalledWith('sync', 'gmb_account', 'acc-1', expect.objectContaining({ status: 'failed', stage: 'transaction' }))
    expect(trackSyncResult).toHaveBeenCalledWith('user-1', false, expect.any(Number))
  })

  describe('review mapping edge cases', () => {
    const table: Array<{ name: string; star: any; expected: number }> = [
    ...
    it.each(table)('maps rating: %s', async (_row) => {
      ...
      const locationsResponse = { locations: [{ name: 'accounts/999/locations/l1', title: 'L1', openInfo: { status: 'OPEN' } }] }
      const reviewsResponse = { reviews: [
        { reviewId: 'rx', starRating: row.star, reviewer: {}, comment: 'txt' },
      ]}
      const questionsResponse = { questions: [] }
      ...
      runSyncTransactionWithRetry.mockImplementation(async (_: any, payload: SyncTransactionPayload) => ({
        success: true,
        sync_id: 'sync-edge',
      ...
      const payload: SyncTransactionPayload = runSyncTransactionWithRetry.mock.calls[0][1]
  ...
  describe('question mapping edge cases', () => {
    it('no topAnswers results in unanswered', async () => {
      ...
      const reviewsResponse = { reviews: [] }
      const questionsResponse = { questions: [
        { name: 'accounts/999/locations/l1/questions/q1', text: 'Q', author: null, createTime: undefined },
      ]}
      ...
      const questionsResponse = { questions: [
        {
          ...
          text: 'Q2',
          topAnswers: [
            { text: 'A1', updateTime: undefined, author: { displayName: null } },
            { text: 'A2', updateTime: '2025-01-01T00:00:00Z', author: { displayName: 'User' } },
          ],
        },
      ]}
  ...
  it('ensures publishSyncProgress order on success and exact cache refresh count', async () => {
    ...
    runSyncTransactionWithRetry.mockResolvedValue({ success: true, sync_id: 'sync-order', locations_synced: 1, reviews_synced: 0, questions_synced: 0 })

    await performTransactionalSync('acc-1', true)

    const sequence = publishSyncProgress.mock.calls.map((c) => `${c[0].stage}:${c[0].status}`)
  ...
  it('supports pagination (extra fetch calls) and empty dataset', async () => {
    ...
    const firstPage = { locations: [{ name: 'accounts/999/locations/l1', title: 'L1', openInfo: { status: 'OPEN' } }], nextPageToken: 'p2' }
    ...
    ;(global.fetch as jest.Mock).mockImplementation((input: RequestInfo | URL) => {
      ...
      if (url.pathname.includes('/reviews')) return Promise.resolve({ ok: true, json: async () => reviewsEmpty })
      if (url.pathname.includes('/questions')) return Promise.resolve({ ok: true, json: async () => questionsEmpty })
      return Promise.resolve({ ok: true, json: async () => ({}) })
    })
    ...
    const locCalls = (global.fetch as jest.Mock).mock.calls.filter(([u]: any[]) => String(u).includes('/locations') && !String(u).includes('/reviews') && !String(u).includes('/questions'))
  ...
  it('empty dataset end-to-end yields empty arrays in transaction', async () => {
    ...
    const result = await performTransactionalSync('acc-1', true)
    expect(result).toEqual({ success: true, sync_id: 'sync-empty', locations_synced: 0, reviews_synced: 0, questions_synced: 0 })
  ...
  it('does not hit unexpected endpoint URLs (strict mock)', async () => {
    ...
    ;(global.fetch as jest.Mock).mockImplementation((input: RequestInfo | URL) => {
      const url = String(input)
      if (url.startsWith('https://mybusinessbusinessinformation.googleapis.com/v1/accounts/999/locations')) {
        return Promise.resolve({ ok: true, json: async () => locationsResponse })
      }
      if (url.startsWith('https://mybusiness.googleapis.com/v4/accounts/999/locations/l1/reviews')) {
        return Promise.resolve({ ok: true, json: async () => reviewsResponse })
      }
      if (url.startsWith('https://mybusinessqanda.googleapis.com/v1/accounts/999/locations/l1/questions')) {
        return Promise.resolve({ ok: true, json: async () => questionsResponse })
      }
      throw new Error('Unexpected URL: ' + url)
    })

    runSyncTransactionWithRetry.mockResolvedValue({ success: true, sync_id: 'sync-strict', locations_synced: 1, reviews_synced: 0, questions_synced: 0 })
    await expect(performTransactionalSync('acc-1', true)).resolves.toBeDefined()
  ...
  it('runs two syncs concurrently and maintains correct behavior and progress', async () => {
    ...
    runSyncTransactionWithRetry.mockResolvedValue({ success: true, sync_id: 'sync-conc', locations_synced: 1, reviews_synced: 0, questions_synced: 0 })
