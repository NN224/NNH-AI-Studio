...
jest.mock('@/lib/gmb/helpers', () => ({
  getValidAccessToken: (...args: any[]) => (helpers.getValidAccessToken as any)(...args),
...
describe('performTransactionalSync - error and exception handling', () => {
  ...
  it('fails cleanly when locations endpoint 500 - emits error stage, no transaction, no cache refresh', async () => {
    ;(global.fetch as jest.Mock).mockResolvedValue({ ok: false, json: async () => ({ error: { message: 'boom' } }) })

    await expect(performTransactionalSync('acc-1', true)).rejects.toThrow()

    // Progress events: error at locations_fetch and complete
    const stages = publishSyncProgress.mock.calls.map((c) => ({ stage: c[0]?.stage, status: c[0]?.status }))
    ...
    expect(logAction).toHaveBeenCalledWith('sync', 'gmb_account', 'acc-1', expect.objectContaining({ status: 'failed', stage: 'locations_fetch' }))
  ...
  it('continues when reviews endpoint 500 - skips reviews for that location and succeeds', async () => {
    ...
    runSyncTransactionWithRetry.mockResolvedValue({ success: true, sync_id: 'sync-ok', locations_synced: 1, reviews_synced: 0, questions_synced: 0 })
    ...
    const stages = publishSyncProgress.mock.calls.map((c) => ({ stage: c[0]?.stage, status: c[0]?.status }))
    expect(stages).toEqual(expect.arrayContaining([expect.objectContaining({ stage: 'reviews_fetch', status: 'completed' })]))
  })
  ...
  it('continues when questions endpoint 500 - skips questions for that location and succeeds', async () => {
    ...
    ;(global.fetch as jest.Mock).mockImplementation((input: RequestInfo | URL) => {
      const url = String(input)
      if (url.includes('/questions')) {
        return Promise.resolve({ ok: false, json: async () => ({ error: { message: 'qfail' } }) })
      }
      if (url.includes('/reviews')) {
    ...
    const stages = publishSyncProgress.mock.calls.map((c) => ({ stage: c[0]?.stage, status: c[0]?.status }))
    expect(stages).toEqual(expect.arrayContaining([expect.objectContaining({ stage: 'questions_fetch', status: 'completed' })]))
  })
  ...
  it('handles malformed API responses (null/undefined arrays, missing fields) without throwing', async () => {
    ...
    ;(global.fetch as jest.Mock).mockImplementation((input: RequestInfo | URL) => {
      ...
      if (url.includes('/questions')) {
        return Promise.resolve({ ok: true, json: async () => ({ questions: [{ name: null, text: 'bad' }] }) })
      }
      return Promise.resolve({ ok: true, json: async () => ({}) })
    })

    runSyncTransactionWithRetry.mockResolvedValue({ success: true, sync_id: 'sync-malformed', locations_synced: 0, reviews_synced: 0, questions_synced: 0 })
  ...
  it('fails when token retrieval fails - emits init error and no transaction', async () => {
    helpers.getValidAccessToken.mockRejectedValueOnce(new Error('token failed'))
    // minimal fetch should not be called in this case, but even if it is, we ensure transaction not called
    ;(global.fetch as jest.Mock).mockResolvedValue({ ok: true, json: async () => ({}) })

    await expect(performTransactionalSync('acc-1', true)).rejects.toThrow('token failed')
    ...
    expect(runSyncTransactionWithRetry).not.toHaveBeenCalled()
    expect(refreshCache).not.toHaveBeenCalled()
    expect(logAction).toHaveBeenCalledWith('sync', 'gmb_account', 'acc-1', expect.objectContaining({ status: 'failed', stage: 'init' }))
    expect(trackSyncResult).toHaveBeenCalledWith('user-1', false, expect.any(Number))
  })
})
