...
describe('Input Validator - Pattern Validation', () => {
  it('should accept matching patterns', () => {
    const result = validatePattern('test@example.com', VALIDATION_PATTERNS.EMAIL);
    expect(result.valid).toBe(true);
  });
  ...
  it('should use custom error message', () => {
    const result = validatePattern('123', /^[a-z]+$/, 'Must contain only lowercase letters');
    expect(result.valid).toBe(false);
    expect(result.message).toBe('Must contain only lowercase letters');
  });

  it('should validate email pattern', () => {
    expect(validatePattern('test@example.com', VALIDATION_PATTERNS.EMAIL).valid).toBe(true);
    expect(validatePattern('invalid', VALIDATION_PATTERNS.EMAIL).valid).toBe(false);
  });

  it('should validate username pattern', () => {
    expect(validatePattern('user123', VALIDATION_PATTERNS.USERNAME).valid).toBe(true);
    expect(validatePattern('ab', VALIDATION_PATTERNS.USERNAME).valid).toBe(false); // Too short
    expect(validatePattern('user@123', VALIDATION_PATTERNS.USERNAME).valid).toBe(false); // Invalid char
  });

  it('should validate password pattern', () => {
    expect(validatePattern('Test123!@#', VALIDATION_PATTERNS.PASSWORD).valid).toBe(true);
    expect(validatePattern('weak', VALIDATION_PATTERNS.PASSWORD).valid).toBe(false);
  });
});
