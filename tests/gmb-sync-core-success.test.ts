...
describe('performTransactionalSync - core success paths', () => {
  ...
  it('omits questions when includeQuestions=false and sends empty questions array', async () => {
    const locationsResponse = {
      locations: [
        { name: 'accounts/999/locations/locA', title: 'Loc A', openInfo: { status: 'OPEN' } },
      ],
    }
    ...
    ;(global.fetch as jest.Mock).mockImplementation((input: RequestInfo | URL) => {
      ...
      if (url.startsWith('https://mybusiness.googleapis.com/v4/accounts/999/locations/locA/reviews')) {
    ...
    const result = await performTransactionalSync('acc-1', false /* includeQuestions */)
    ...
    const questionCalls = (global.fetch as jest.Mock).mock.calls.filter(([u]: any[]) =>
  ...
  it('handles multiple locations (2-3) with mixed missing fields and maps correctly', async () => {
    const locationsResponse = {
      locations: [
        ...
        },
        {
          ...
          metadata: { totalReviewCount: 4, starRating: 3.2, status: 'verified' },
    ...
    const reviewsLoc1 = { reviews: [
      { reviewId: 'r1', starRating: 'STAR_FOUR', comment: 'ok', createTime: '2025-02-01T00:00:00Z' },
    ]}
    const reviewsLoc2 = { reviews: [
      { name: 'accounts/999/locations/loc2/reviews/r2', starRating: 2, comment: 'meh' },
    ]}

    const questionsLoc1 = { questions: [
      { name: 'accounts/999/locations/loc1/questions/q1', text: 'Q1' },
    ]}
    const questionsLoc2 = { questions: [
      { name: 'accounts/999/locations/loc2/questions/q2', text: 'Q2', topAnswers: [{ text: 'A2' }] },
    ]}
    ...
    expect(result).toEqual({ success: true, sync_id: 'sync-multi', locations_synced: 2, reviews_synced: 2, questions_synced: 2 })
    ...
    expect(payload.reviews).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ review_id: 'r1', google_location_id: 'accounts/999/locations/loc1', rating: 4 }),
        expect.objectContaining({ review_id: 'r2', google_location_id: 'accounts/999/locations/loc2', rating: 2 }),
      ]),
    )

    // Questions mapping
    expect(payload.questions.find((q) => q.question_id === 'q1')?.status).toBe('unanswered')
    expect(payload.questions.find((q) => q.question_id === 'q2')?.status).toBe('answered')

    // Progress stages include questions when enabled
    const stages = publishSyncProgress.mock.calls.map((c) => c[0]?.stage)
    expect(stages).toEqual(
      expect.arrayContaining(['init', 'locations_fetch', 'reviews_fetch', 'questions_fetch', 'transaction', 'cache_refresh', 'complete']),
    )
